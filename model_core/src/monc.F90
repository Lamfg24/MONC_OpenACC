...
module monc_mod
  ...
  use monc_component_mod, only : component_descriptor_type, component_descriptor_type_v1_array
  ...
  use state_mod
  use registry_mod, only : register_component, execute_initialisation_callbacks, execute_finalisation_callbacks
...
#ifndef TEST_MODE
  use netcdf, only : nf90_nowrite, nf90_open, nf90_inq_varid, nf90_get_var, nf90_close, nf90_noerr, nf90_strerror
#else
  use dummy_netcdf_mod, only : nf90_nowrite, nf90_open, nf90_inq_varid, nf90_get_var, nf90_close, nf90_noerr, nf90_strerror
#endif
...
contains
  ...
  subroutine monc_core_bootstrap()!(component_descriptions)
    ...
    integer :: ierr, myrank, nb_proc, moncs_per_io_server, provided_threading, &
               selected_threading_mode, iter, intnum
    logical :: i_am_monc_process
    logical :: io_continuation, logicnum
    real(kind=DEFAULT_PRECISION) :: reconfig_initial_time, realnum
    ...
    ! Setted to NaN values
    state%options_database_real = IEEE_VALUE(state%options_database_real, IEEE_QUIET_NAN)

    ! Load model configuration
    reconfig_initial_time = 0.0_DEFAULT_PRECISION ! set locally, should precede load_model_configuration
    call load_model_configuration(state, &
                                  io_continuation, reconfig_initial_time) ! load in options_database_string structure online commands and features

    do iter = 1,state%config_args
      if (state%options_database_string(iter,1) .eq. "logging") then
        read(state%options_database_string(iter,2),*) intnum
        call log_set_logging_level(intnum)
      else if (state%options_database_string(iter,1) .eq. "iobridge_enabled") then
        read(state%options_database_string(iter,2),*) logicnum
        state%io_server_enabled = logicnum
      else if (state%options_database_string(iter,1) .eq. "time_frequency_enabled") then
        read(state%options_database_string(iter,2),*) logicnum
        state%time_frequency_enabled = logicnum
      else if (state%options_database_string(iter,1) .eq. "checkpoint_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%checkpoint_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_0d_write_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_0d_write_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_1d_write_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_1d_write_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_2d_write_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_2d_write_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_3d_write_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_3d_write_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_averaged_sample_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_averaged_sample_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "diagnostic_file_averaged_write_frequency") then
        read(state%options_database_string(iter,2),*) intnum
        state%diagnostic_file_averaged_write_frequency = intnum
      else if (state%options_database_string(iter,1) .eq. "init_to_apply_random_noise") then
        read(state%options_database_string(iter,2),*) logicnum
        state%init_to_apply_random_noise = logicnum
      else if (state%options_database_string(iter,1) .eq. "time_to_apply_random_noise") then
        read(state%options_database_string(iter,2),*) realnum
        state%time_to_apply_random_noise = realnum
      else if (state%options_database_string(iter,1) .eq. "duration_random_noise") then
        read(state%options_database_string(iter,2),*) realnum
        state%duration_random_noise = realnum
      else if (state%options_database_string(iter,1) .eq. "constant_w_profile_timer") then
        read(state%options_database_string(iter,2),*) realnum
        state%constant_w_profile_timer = realnum
      end if
    end do
    call perform_options_compatibility_checks(state)

    if (state%diagnostic_file_averaged_write_frequency .eq. 0 .or. state%diagnostic_file_averaged_sample_frequency .eq. 0) then
      state%dim_var_to_mean = 0
    else
      state%dim_var_to_mean = int(state%diagnostic_file_averaged_write_frequency/state%diagnostic_file_averaged_sample_frequency)
      print*,"state%dim_var_to_mean = ",state%dim_var_to_mean
    end if

    ! Check on io_server settings and start MONC
    if (state%io_server_enabled) then
      call mpi_comm_size(MPI_COMM_WORLD, nb_proc, ierr)
      if (nb_proc==1) call log_log(LOG_ERROR, &
           "Run with 1 process, With IO server enabled then the minimum process size is 2 (1 for IO, 1 for MONC)")
      call get_io_configuration(state, io_server_config_file, &
                                moncs_per_io_server)
      call split_communicator_into_monc_and_io(moncs_per_io_server, state%parallel%monc_communicator, &
      ...
      if (.not. i_am_monc_process) then
        call io_server_run(state, state%parallel%io_communicator, &
                           provided_threading, nb_proc, io_continuation, reconfig_initial_time, io_server_config_file, myrank)
      else
        call monc_run(state)
      end if
    !else
  ...
  subroutine load_model_configuration(state, &
                                      io_continuation, reconfig_initial_time)
    type(model_state_type), intent(inout) :: state
    !type(hashmap_type), intent(inout) :: options_database
    logical, intent(out) :: io_continuation
    real(kind=DEFAULT_PRECISION), intent(inout) :: reconfig_initial_time

    integer :: ncdf_id, time_id
    integer, dimension(2) :: shape_arr
    real(kind=DEFAULT_PRECISION) ::  time_checkpoint
    character(len=200) :: mcf_filename

    call load_command_line_into_options_database(state)

      !Cold start from mcf config file.
      !if (options_has_key(options_database, "config")) then
      if (state%options_database_string(1,1) .eq. "config") then
        state%continuation_run=.false.
        io_continuation=.false.
        call log_master_log(LOG_INFO, "This cycle is a cold start using config: '"//&
                           trim(state%options_database_string(1,1))//"'")
        call log_master_newline()

        mcf_filename = state%options_database_string(1,2)
        call parse_configuration_file(state, mcf_filename)

        call mkdir_results_folder(state, mcf_filename, .false.)
      ...
      else if (state%options_database_string(1,1) .eq. "reconfig" .and. &
             state%options_database_string(2,1) .eq. "checkpoint") then
!       state%reconfig_run=.true.    ! this is specific to the initial cycle of this kind of run
        ...
        call log_master_log(LOG_INFO, "This cycle is a reconfigured start using config: '"//&
              trim(state%options_database_string(1,2))//&
             "' from checkpoint: '"//trim(state%options_database_string(2,2))//"'")

        call check(nf90_open(state%options_database_string(2,2), nf90_nowrite, ncdf_id))
        call check(nf90_inq_varid(ncdf_id, "time", time_id))
        call check(nf90_get_var(ncdf_id, time_id, time_checkpoint))
        ...
        call mkdir_results_folder(state, mcf_filename, .true.)

        ! Check to see if retain_model_time was loaded in via command line options
!       if (options_database_string(3,1) .eq. "retain_model_time") then
!         if (options_database_string(3,2) .eq. ".true.") then
...
!     end if
  ...
  !! @param options_database The options database
  subroutine perform_options_compatibility_checks(state)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state

    integer :: iter
    logical :: diag_component = .false.

    !> If conditional diagnostics are operating, both components should be enabled.
    do iter = 1,state%config_args
      if (state%options_database_string(iter,1) .eq. "conditional_diagnostics_column_enabled") then
        diag_component = .true.
      else if (state%options_database_string(iter,1) .eq. "conditional_diagnostics_whole_enabled") then
        diag_component = .true.
      end if
    end do
    if (diag_component .eqv. .false.) then
  ...
  subroutine monc_run(state)
    !type(component_descriptor_type_v1_array), intent(inout) :: component_descriptions
    type(model_state_type), intent(inout) :: state

    integer :: ierr, total_size, iter
    double precision :: end_time, timestepping_time, modeldump_time
    logical :: enable_run = .false.
    ...
    !call init_registry(state%options_database_real, state%options_database_string) ! Initialise the registry

    call fill_registry_with_components(state%options_database_string, state)
    ...
    do iter = 1,state%config_args
      if (.not. state%options_database_string(iter,1) .eq. "norun") then
        if (.not. state%options_database_string(iter,2) .eq. ".true.") then
          enable_run = .true.
        end if
      end if
    end do

    !print*,"enable_run = ",enable_run
    if (enable_run .eqv. .true.) then
  ...
  subroutine fill_registry_with_components(options_database_string, current_state)
    !type(hashmap_type), intent(inout) :: options_database
    ...
    integer :: iter1, iter2
    logical :: logicnum, component_enabled

    !class(*), pointer :: raw_data
    !type(iterator_type) :: iterator
    component_enabled=.false.
    !print*,"size(component_descriptions%name_array) = ",size(component_descriptions%name_array)
    !do iter1 = 1, size(component_descriptions%name_array)
      !print*,"component_descriptions%name_array(iter1) = ",component_descriptions%name_array(iter1)
    !  do iter2 = 1,current_state%config_args
    !    if (options_database_string(iter2,1) .eq. trim(component_descriptions%name_array(iter1))//"_enabled") then
    !      read(options_database_string(iter2,2),*) logicnum
    !      component_enabled= logicnum
    !      !print*,"component_enabled = ",component_enabled
    !    end if
    !  end do
    !  if (component_enabled .eqv. .false.) then
    !    call log_master_log(LOG_WARN, "No enabled configuration for component " &
    !         //trim(component_descriptions%name_array(iter1))//" therefore disabling this")
    !  end if
    !end do

    call register_component(current_state)
    ...
    !      call register_component(options_database, raw_data) !!!! IMPORTANT
    !    class default
  ...
  !! @param options_database The options database
  subroutine get_io_configuration(state, ioserver_configuration_file, moncs_per_io_server)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    character(len=LONG_STRING_LENGTH), intent(out) :: ioserver_configuration_file
    integer, intent(out) :: moncs_per_io_server
   
    integer :: myrank, ierr, iter, intnum

    do iter = 1,state%config_args
      if (state%options_database_string(iter,1) .eq. "ioserver_configuration_file") then
      !print*,"options_database_string(iter,2) = ",options_database_string(iter,2)
        ioserver_configuration_file = state%options_database_string(iter,2)
      else if (state%options_database_string(iter,1) .eq. "moncs_per_io_server") then
        read(state%options_database_string(iter,2),*) intnum
        moncs_per_io_server = intnum
        !print*,"intnum = ",intnum
      end if
    end do
  ...
  integer function get_mpi_threading_mode()
    ...
    if (status == 0 .and. conv_is_logical(trim(thread_multiple_config_value))) then
      if (conv_to_logical(trim(thread_multiple_config_value))) then
        get_mpi_threading_mode=MPI_THREAD_MULTIPLE ! = 3
      else
        get_mpi_threading_mode=MPI_THREAD_SERIALIZED ! = 2
      end if
    else
      get_mpi_threading_mode=MPI_THREAD_SERIALIZED ! = 2
    end if
  ...
  subroutine mkdir_results_folder(state, mcf_filename, opt_reconf)
    type(model_state_type), intent(inout) :: state
    character(len=200), intent(in) :: mcf_filename
    logical :: opt_reconf

    integer :: iter
    integer :: values(8)
    character(len=200) :: results_path, mcf_path, time_folder

    do iter = 1,state%config_args
      if (state%options_database_string(iter,1) .eq. "results_path") then
        results_path = state%options_database_string(iter,2)
      end if
    end do

    call date_and_time(values=values)

    time_folder = trim(conv_to_string(values(1))) // "_" // trim(conv_to_string(values(2))) // "_" // &
                  trim(conv_to_string(values(3))) // "_" // trim(conv_to_string(values(5))) // "_" // &
                  trim(conv_to_string(values(6))) // "_" // trim(conv_to_string(values(7)))

    results_path = trim(results_path) // "/" // trim(time_folder)
    if (opt_reconf .eqv. .false.) then
      results_path = trim(results_path) // "_start"
    else
      results_path = trim(results_path) // "_restart"
    end if
    mcf_path = trim(results_path) // "/mcf"
    state%checkpoint_path = trim(results_path) // "/checkpoint_files"
    state%diagnostic_path = trim(results_path) // "/diagnostic_files"

    if (state%parallel%my_global_rank == 1) then
      call execute_command_line("mkdir " // results_path)
      call execute_command_line("mkdir " // mcf_path)
      call execute_command_line("cp " // mcf_filename // mcf_path )
      call execute_command_line("mkdir " // state%checkpoint_path)
      call execute_command_line("mkdir " // state%diagnostic_path)
    end if
  end subroutine mkdir_results_folder
