...
contains
  ...
  subroutine monc_core_bootstrap()!(component_descriptions)
    ...
    call load_model_configuration(state, state%options_database_real, state%options_database_string, &
    ...
    call perform_options_compatibility_checks(state, state%options_database_string)
    ...
    if (state%io_server_enabled) then
      ...
      call get_io_configuration(state, state%options_database_string, io_server_config_file, &
                                moncs_per_io_server)
      ...
      if (.not. i_am_monc_process) then
        call io_server_run(state, state%options_database_real, state%options_database_string, state%parallel%io_communicator, &
  ...
  subroutine load_model_configuration(state, options_database_real, options_database_string, &
    ...
    integer :: iter, ncdf_id, time_id
    integer, dimension(2) :: shape_arr
    real(kind=DEFAULT_PRECISION) ::  time_checkpoint

    call load_command_line_into_options_database(state, options_database_real, options_database_string)
      ...
      if (state%options_database_string(1,1) .eq. "config") then
        ...
        call parse_configuration_file(state, options_database_real, options_database_string, options_database_string(1,2))
...
!     end if
  ...
  !! @param options_database The options database
  subroutine perform_options_compatibility_checks(state, options_database_string)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    character(len=STRING_LENGTH), dimension(1000,2), intent(inout) :: options_database_string
    ...
    do iter = 1,state%config_args
      if (options_database_string(iter,1) .eq. "conditional_diagnostics_column_enabled") then
        diag_component = .true.
      else if (options_database_string(iter,1) .eq. "conditional_diagnostics_whole_enabled") then
        diag_component = .true.
      end if
    end do
  ...
  !! @param options_database The options database
  subroutine get_io_configuration(state, options_database_string, ioserver_configuration_file, moncs_per_io_server)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    character(len=STRING_LENGTH), dimension(1200,2), intent(inout) :: options_database_string
    ...
    do iter = 1,state%config_args
      if (options_database_string(iter,1) .eq. "ioserver_configuration_file") then
      !print*,"options_database_string(iter,2) = ",options_database_string(iter,2)
        ioserver_configuration_file = options_database_string(iter,2)
      else if (options_database_string(iter,1) .eq. "moncs_per_io_server") then
        read(options_database_string(iter,2),*) intnum
