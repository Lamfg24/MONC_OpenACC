...
contains
  ...
  ! @param temperature of freezing in Kelvin
  ! @param pressure in mb
  real(kind=DEFAULT_PRECISION) function qisaturation(temperature, pressure)
    real(kind=DEFAULT_PRECISION), intent(in) :: temperature, pressure

    qisaturation = qis1/(pressure*exp(qis2*(temperature - tk0c)/(temperature - qis3)) - qis4)
