...
module grids_mod
  ...
  type, public :: vertical_grid_configuration_type
     real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: &
          ...
          w_init,&     !< Initial profile of w
          prefn,& !< Reference pressure (Pa)
          pdiff,& !< Difference between pressure levels (Pa)
          prefrcp,& ! (Psurf/Pz)^rcp
          rprefrcp,& ! values to return from potential temperature (K) to temperature (K)
          ...
          qv_rand, ql_rand, qr_rand, qi_rand, qs_rand, qg_rand, &
          qAitkenSolMass_rand, qAccumSolMass_rand, qAccumInsolMass_rand, &
          qCoarseSolMass_rand, qCoarseDustMass_rand, &
          theta_force, & !<profile of forcing term for theta
          u_force, & !<profile of forcing term for u
          v_force, & !<profile of forcing term for v
          qv_force, ql_force, qr_force, qi_force, qs_force, qg_force, &
          qAitkenSolMass_force, qAccumSolMass_force, qAccumInsolMass_force, &
          qCoarseSolMass_force, qCoarseDustMass_force, &
          w_up,    & !<profile of updraft threshold
          w_dwn,   & !<profile of downdraft threshold
          w_rand,  &   !<profile of amplitude of w perturbations
          qv_init, olqvbar, olzqvbar, &
          ql_init, olqlbar, olzqlbar, &
          qr_init, olqrbar, olzqrbar, &
          qi_init, olqibar, olzqibar, &
          qs_init, olqsbar, olzqsbar, &
          qg_init, olqgbar, olzqgbar, &
          qAitkenSolMass_init, olqAitkenSolMassbar, olzqAitkenSolMassbar, &
          qAccumSolMass_init, olqAccumSolMassbar, olzqAccumSolMassbar, &
          qAccumInsolMass_init, olqAccumInsolMassbar, olzqAccumInsolMassbar, &
          qCoarseSolMass_init, olqCoarseSolMassbar, olzqCoarseSolMassbar, &
          qCoarseDustMass_init, olqCoarseDustMassbar, olzqCoarseDustMassbar, &
          nl_init, olnlbar, olznlbar, &
          nr_init, olnrbar, olznrbar, &
          ni_init, olnibar, olznibar, &
          ns_init, olnsbar, olznsbar, &
          ng_init, olngbar, olzngbar, &
          nAitkenSolNumber_init, olnAitkenSolNumberbar, olznAitkenSolNumberbar, &
          nAccumSolNumber_init, olnAccumSolNumberbar, olznAccumSolNumberbar, &
          nAccumInsolNumber_init, olnAccumInsolNumberbar, olznAccumInsolNumberbar, &
          nCoarseSolNumber_init, olnCoarseSolNumberbar, olznCoarseSolNumberbar, &
          nCoarseDustnumber_init, olnCoarseDustnumberbar, olznCoarseDustnumberbar
     ...
     real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: hgd, origional_vertical_grid_interp_altitudes
     real(kind=DEFAULT_PRECISION) :: czn, zlogm, zlogth, vk_on_zlogm
     integer, dimension(:), allocatable :: kgd, origional_vertical_grid_interp_levels
     integer :: kdmpmin, x_size, y_size, z_size
