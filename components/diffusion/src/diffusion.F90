...
contains
  ...
  subroutine initialisation_callback_diffusion(current_state)
    ...
    allocate(current_state%th_diffusion(z_size))
    allocate(current_state%qv_diffusion(z_size))
    allocate(current_state%ql_diffusion(z_size))
    allocate(current_state%qr_diffusion(z_size))
    allocate(current_state%qi_diffusion(z_size))
    allocate(current_state%qs_diffusion(z_size))
    allocate(current_state%qg_diffusion(z_size))
    allocate(current_state%qAitkenSolMass_diffusion(z_size))
    allocate(current_state%qAccumSolMass_diffusion(z_size))
    allocate(current_state%qAccumInsolMass_diffusion(z_size))
    allocate(current_state%qCoarseSolMass_diffusion(z_size))
    allocate(current_state%qCoarseDustMass_diffusion(z_size))

    allocate(current_state%nl_diffusion(z_size))
    allocate(current_state%nr_diffusion(z_size))
    allocate(current_state%ni_diffusion(z_size))
    allocate(current_state%ns_diffusion(z_size))
    allocate(current_state%ng_diffusion(z_size))
    allocate(current_state%nAitkenSolNumber_diffusion(z_size))
    allocate(current_state%nAccumSolNumber_diffusion(z_size))
    allocate(current_state%nAccumInsolNumber_diffusion(z_size))
    allocate(current_state%nCoarseSolNumber_diffusion(z_size))
    allocate(current_state%nCoarseDustnumber_diffusion(z_size))
  ...
  subroutine finalisation_callback_diffusion(current_state)
    ...
    if (allocated(current_state%th_diffusion)) deallocate(current_state%th_diffusion)
    if (allocated(current_state%qv_diffusion)) deallocate(current_state%qv_diffusion)
    if (allocated(current_state%ql_diffusion)) deallocate(current_state%ql_diffusion)
    if (allocated(current_state%qr_diffusion)) deallocate(current_state%qr_diffusion)
    if (allocated(current_state%qi_diffusion)) deallocate(current_state%qi_diffusion)
    if (allocated(current_state%qs_diffusion)) deallocate(current_state%qs_diffusion)
    if (allocated(current_state%qg_diffusion)) deallocate(current_state%qg_diffusion)
    if (allocated(current_state%qAitkenSolMass_diffusion)) deallocate(current_state%qAitkenSolMass_diffusion)
    if (allocated(current_state%qAccumSolMass_diffusion)) deallocate(current_state%qAccumSolMass_diffusion)
    if (allocated(current_state%qAccumInsolMass_diffusion)) deallocate(current_state%qAccumInsolMass_diffusion)
    if (allocated(current_state%qCoarseSolMass_diffusion)) deallocate(current_state%qCoarseSolMass_diffusion)
    if (allocated(current_state%qCoarseDustMass_diffusion)) deallocate(current_state%qCoarseDustMass_diffusion)

    if (allocated(current_state%nl_diffusion)) deallocate(current_state%nl_diffusion)
    if (allocated(current_state%nr_diffusion)) deallocate(current_state%nr_diffusion)
    if (allocated(current_state%ni_diffusion)) deallocate(current_state%ni_diffusion)
    if (allocated(current_state%ns_diffusion)) deallocate(current_state%ns_diffusion)
    if (allocated(current_state%ng_diffusion)) deallocate(current_state%ng_diffusion)
    if (allocated(current_state%nAitkenSolNumber_diffusion)) deallocate(current_state%nAitkenSolNumber_diffusion)
    if (allocated(current_state%nAccumSolNumber_diffusion)) deallocate(current_state%nAccumSolNumber_diffusion)
    if (allocated(current_state%nAccumInsolNumber_diffusion)) deallocate(current_state%nAccumInsolNumber_diffusion)
    if (allocated(current_state%nCoarseSolNumber_diffusion)) deallocate(current_state%nCoarseSolNumber_diffusion)
    if (allocated(current_state%nCoarseDustnumber_diffusion)) deallocate(current_state%nCoarseDustnumber_diffusion)
...
#ifdef DIFF_QV
    call general_diffusion(current_state, current_state%zqv, current_state%sqv, local_y, local_x, current_state%qv_diffusion)
#endif
#ifdef DIFF_QL
    call general_diffusion(current_state, current_state%zql, current_state%sql, local_y, local_x, current_state%ql_diffusion)
#endif
#ifdef DIFF_QR
    call general_diffusion(current_state, current_state%zqr, current_state%sqr, local_y, local_x, current_state%qr_diffusion)
#endif
#ifdef DIFF_QI
    call general_diffusion(current_state, current_state%zqi, current_state%sqi, local_y, local_x, current_state%qi_diffusion)
#endif
#ifdef DIFF_QS
    call general_diffusion(current_state, current_state%zqs, current_state%sqs, local_y, local_x, current_state%qs_diffusion)
#endif
#ifdef DIFF_QG
    call general_diffusion(current_state, current_state%zqg, current_state%sqg, local_y, local_x, current_state%qg_diffusion)
#endif
#ifdef DIFF_QAIKT
    call general_diffusion(current_state, current_state%zqAitkenSolMass, current_state%sqAitkenSolMass, &
                           local_y, local_x, current_state%qAitkenSolMass_diffusion)
#endif
#ifdef DIFF_QACCSOL
    call general_diffusion(current_state, current_state%zqAccumSolMass, current_state%sqAccumSolMass, &
                           local_y, local_x, current_state%qAccumSolMass_diffusion)
#endif
#ifdef DIFF_QACCINSOL
    call general_diffusion(current_state, current_state%zqAccumInsolMass, current_state%sqAccumInsolMass, &
                           local_y, local_x, current_state%qAccumInsolMass_diffusion)
#endif
#ifdef DIFF_QCOARSOL
    call general_diffusion(current_state, current_state%zqCoarseSolMass, current_state%sqCoarseSolMass, &
                           local_y, local_x, current_state%qCoarseSolMass_diffusion)
#endif
#ifdef DIFF_QCOARDU
    call general_diffusion(current_state, current_state%zqCoarseDustMass, current_state%sqCoarseDustMass, &
                           local_y, local_x, current_state%qCoarseDustMass_diffusion)
#endif

#ifdef DIFF_NL
    call general_diffusion(current_state, current_state%znl, current_state%snl, local_y, local_x, current_state%nl_diffusion)
#endif
#ifdef DIFF_NR
    call general_diffusion(current_state, current_state%znr, current_state%snr, local_y, local_x, current_state%nr_diffusion)
#endif
#ifdef DIFF_NI
    call general_diffusion(current_state, current_state%zni, current_state%sni, local_y, local_x, current_state%ni_diffusion)
#endif
#ifdef DIFF_NS
    call general_diffusion(current_state, current_state%zns, current_state%sns, local_y, local_x, current_state%ns_diffusion)
#endif
#ifdef DIFF_NG
    call general_diffusion(current_state, current_state%zng, current_state%sng, local_y, local_x, current_state%ng_diffusion)
#endif
#ifdef DIFF_NAIKT
    call general_diffusion(current_state, current_state%znAitkenSolNumber, current_state%snAitkenSolNumber, &
                           local_y, local_x, current_state%nAitkenSolNumber_diffusion)
#endif
#ifdef DIFF_NACCSOL
    call general_diffusion(current_state, current_state%znAccumSolNumber, current_state%snAccumSolNumber, &
                           local_y, local_x, current_state%nAccumSolNumber_diffusion)
#endif
#ifdef DIFF_NACCINSOL
    call general_diffusion(current_state, current_state%znAccumInsolNumber, current_state%snAccumInsolNumber, &
                           local_y, local_x, current_state%nAccumInsolNumber_diffusion)
#endif
#ifdef DIFF_NCOARSOL
    call general_diffusion(current_state, current_state%znCoarseSolNumber, current_state%snCoarseSolNumber, &
                           local_y, local_x, current_state%nCoarseSolNumber_diffusion)
#endif
#ifdef DIFF_NCOARDU
    call general_diffusion(current_state, current_state%znCoarseDustnumber, current_state%snCoarseDustnumber, &
                           local_y, local_x, current_state%nCoarseDustnumber_diffusion)
#endif
  ...
  subroutine perform_th_diffusion(current_state, local_y, local_x)
    ...
    call general_diffusion(current_state, current_state%zth, current_state%sth, local_y, local_x, current_state%th_diffusion)
    ...
    if (current_state%use_anelastic_equations) then
     ! This code only needs to be executed if anelastic, otherwise THREF is constant and the increment calculated here is zero
      do k=2, current_state%local_grid%size(Z_INDEX)
        ...
        current_state%th_diffusion(k)=current_state%th_diffusion(k)+adjustment
      end do
    end if
