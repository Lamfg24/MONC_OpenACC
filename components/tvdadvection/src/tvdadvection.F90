...
contains
  ...
  subroutine initialisation_callback_tvdadvection(current_state)
    ...
    if (.not. allocated(current_state%u_advection)) allocate(current_state%u_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%v_advection)) allocate(current_state%v_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%w_advection)) allocate(current_state%w_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%th_advection)) allocate(current_state%th_advection(current_state%global_grid%size(Z_INDEX)))

    if (.not. allocated(current_state%qv_advection)) allocate(current_state%qv_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%ql_advection)) allocate(current_state%ql_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qr_advection)) allocate(current_state%qr_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qi_advection)) allocate(current_state%qi_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qs_advection)) allocate(current_state%qs_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qg_advection)) allocate(current_state%qg_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qAitkenSolMass_advection)) &
      allocate(current_state%qAitkenSolMass_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qAccumSolMass_advection)) &
      allocate(current_state%qAccumSolMass_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qAccumInsolMass_advection)) &
      allocate(current_state%qAccumInsolMass_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qCoarseSolMass_advection)) &
      allocate(current_state%qCoarseSolMass_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%qCoarseDustMass_advection)) &
      allocate(current_state%qCoarseDustMass_advection(current_state%global_grid%size(Z_INDEX)))

    if (.not. allocated(current_state%nl_advection)) allocate(current_state%nl_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nr_advection)) allocate(current_state%nr_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%ni_advection)) allocate(current_state%ni_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%ns_advection)) allocate(current_state%ns_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%ng_advection)) allocate(current_state%ng_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nAitkenSolNumber_advection)) &
      allocate(current_state%nAitkenSolNumber_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nAccumSolNumber_advection)) &
      allocate(current_state%nAccumSolNumber_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nAccumInsolNumber_advection)) &
      allocate(current_state%nAccumInsolNumber_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nCoarseSolNumber_advection)) &
      allocate(current_state%nCoarseSolNumber_advection(current_state%global_grid%size(Z_INDEX)))
    if (.not. allocated(current_state%nCoarseDustnumber_advection)) &
      allocate(current_state%nCoarseDustnumber_advection(current_state%global_grid%size(Z_INDEX)))

    allocate(tracer_advection(current_state%global_grid%size(Z_INDEX), current_state%n_tracers))
  ...
  subroutine finalisation_callback_tvdadvection(current_state)
    ...
    if (allocated(current_state%u_advection)) deallocate(current_state%u_advection)
    if (allocated(current_state%v_advection)) deallocate(current_state%v_advection)
    if (allocated(current_state%w_advection)) deallocate(current_state%w_advection)
    if (allocated(current_state%th_advection)) deallocate(current_state%th_advection)

    if (allocated(current_state%qv_advection)) deallocate(current_state%qv_advection)
    if (allocated(current_state%ql_advection)) deallocate(current_state%ql_advection)
    if (allocated(current_state%qr_advection)) deallocate(current_state%qr_advection)
    if (allocated(current_state%qi_advection)) deallocate(current_state%qi_advection)
    if (allocated(current_state%qs_advection)) deallocate(current_state%qs_advection)
    if (allocated(current_state%qg_advection)) deallocate(current_state%qg_advection)
    if (allocated(current_state%qAitkenSolMass_advection)) deallocate(current_state%qAitkenSolMass_advection)
    if (allocated(current_state%qAccumSolMass_advection)) deallocate(current_state%qAccumSolMass_advection)
    if (allocated(current_state%qAccumInsolMass_advection)) deallocate(current_state%qAccumInsolMass_advection)
    if (allocated(current_state%qCoarseSolMass_advection)) deallocate(current_state%qCoarseSolMass_advection)
    if (allocated(current_state%qCoarseDustMass_advection)) deallocate(current_state%qCoarseDustMass_advection)

    if (allocated(current_state%nl_advection)) deallocate(current_state%nl_advection)
    if (allocated(current_state%nr_advection)) deallocate(current_state%nr_advection)
    if (allocated(current_state%ni_advection)) deallocate(current_state%ni_advection)
    if (allocated(current_state%ns_advection)) deallocate(current_state%ns_advection)
    if (allocated(current_state%ng_advection)) deallocate(current_state%ng_advection)
    if (allocated(current_state%nAitkenSolNumber_advection)) deallocate(current_state%nAitkenSolNumber_advection)
    if (allocated(current_state%nAccumSolNumber_advection)) deallocate(current_state%nAccumSolNumber_advection)
    if (allocated(current_state%nAccumInsolNumber_advection)) deallocate(current_state%nAccumInsolNumber_advection)
    if (allocated(current_state%nCoarseSolNumber_advection)) deallocate(current_state%nCoarseSolNumber_advection)
    if (allocated(current_state%nCoarseDustnumber_advection)) deallocate(current_state%nCoarseDustnumber_advection)
  ...
  subroutine timestep_callback_tvdadvection(current_state)
    ...
    anelastic_enabled = current_state%use_anelastic_equations
...
#ifdef U_ACTIVE
    call advect_u(current_state, current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
...
#ifdef V_ACTIVE
    call advect_v(current_state, current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
...
#ifdef W_ACTIVE
    call advect_w(current_state, current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
  ...
  subroutine advect_q_fields(current_state, profile_diagnostics_enabled)
    ...
    if (q_active > 0) then
#ifdef TVDADV_QV
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
      ...
      current_state%qv_advection=current_state%sqv%data(:, current_state%column_local_y, current_state%column_local_x)
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%ql_advection=current_state%sql%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%qr_advection=current_state%sqr%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%qi_advection=current_state%sqi%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%qs_advection=current_state%sqs%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%qg_advection=current_state%sqg%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%qCoarseDustMass_advection = &
        current_state%sqCoarseDustMass%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nl_advection=current_state%snl%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nr_advection=current_state%snr%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%ni_advection=current_state%sni%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%ns_advection=current_state%sns%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%ng_advection=current_state%sng%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
      ...
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nAitkenSolNumber_advection = &
        current_state%snAitkenSolNumber%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
#endif
#ifdef TVDADV_NACCSOL
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nAccumSolNumber_advection = &
        current_state%snAccumSolNumber%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
#endif
#ifdef TVDADV_NACCINSOL
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nAccumInsolNumber_advection = &
        current_state%snAccumInsolNumber%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
#endif
#ifdef TVDADV_NCOARSOL
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nCoarseSolNumber_advection = &
        current_state%snCoarseSolNumber%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
#endif
#ifdef TVDADV_NCOARDU
      call advect_scalar_field_q(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u, &
            ...
            current_state%global_grid, current_state%local_grid, current_state%parallel, &
            current_state%halo_column, current_state%field_stepping)
      current_state%nCoarseDustnumber_advection = &
        current_state%snCoarseDustnumber%data(:, current_state%column_local_y, current_state%column_local_x)
      !if (profile_diagnostics_enabled) then
      !  tvd_dgs_terms%adv_qCoarseDustMass_dgs(:, current_state%column_local_y, current_state%column_local_x) =  &
      !        flux_z(:)
      !end if
  ...
  subroutine advect_theta(current_state, profile_diagnostics_enabled)
    ...
    if (current_state%th%active) then
      call advect_scalar_field(current_state%column_local_y, current_state%column_local_x, dtm, current_state%u,&
           ...
           current_state%local_grid, current_state%parallel, current_state%halo_column, current_state%field_stepping)
      current_state%th_advection=current_state%sth%data(:, current_state%column_local_y, current_state%column_local_x)
      if (profile_diagnostics_enabled) then !  a revoir
           ! NOTE: flux_z is declared at the top of module and then passed into ultflx, through argument
           !       list in advect_scalar_field.
  ...
  subroutine advect_scalar_field_q(y_local_index, x_local_index, dt, u, v, w, z_q_field, q_field, source_field, &
       global_grid, local_grid, parallel, halo_column, field_stepping)
    ...
    if (.not. halo_column) then
      call differentiate_face_values_q(y_local_index, x_local_index, u, v, w, y_local_index, x_local_index, source_field, &
           local_grid, global_grid, q_field%flux_previous_y, q_field%flux_previous_x(:,y_local_index), &
           global_grid%configuration%vertical%tzc1, global_grid%configuration%vertical%tzc2, .false.)
    end if
    ...
    call register_y_flux_wrap_send_if_required(y_local_index, q_field, parallel, local_grid)
    call complete_y_flux_wrap_send_if_required(y_local_index, q_field, parallel, local_grid)
  end subroutine advect_scalar_field_q

  !> Advects the U flow field
  subroutine advect_u(current_state, y_local_index, x_local_index, dt, u, v, w, zf, su, global_grid, local_grid, parallel, &
    ...
    if (.not. halo_column) then
      ...
      current_state%u_advection=su%data(:, y_local_index, x_local_index)
    end if
  ...
  subroutine advect_v(current_state, y_local_index, x_local_index, dt, u, v, w, zf, sv, global_grid, local_grid, parallel, &
       halo_column, field_stepping)
    ...
    if (.not. halo_column) then
      call differentiate_face_values(1, 1, interpolated_fields(u_index), interpolated_fields(v_index), &
           ...
           global_grid%configuration%vertical%tzc1, global_grid%configuration%vertical%tzc2, .true.)
      current_state%v_advection=sv%data(:, y_local_index, x_local_index)
    end if
  ...
  subroutine advect_w(current_state, y_local_index, x_local_index, dt, u, v, w, zf, sw, global_grid, local_grid, parallel, &
       halo_column, field_stepping)
    ...
    if (.not. halo_column) then
      ...
      current_state%w_advection=sw%data(:, y_local_index, x_local_index)
    end if
  ...
  subroutine differentiate_face_values_q(y_flow_index, x_flow_index, u, v, w, y_source_index, x_source_index, source_field, &
       local_grid, global_grid, flux_y_previous, flux_x_previous, tzc1, tzc2, differentiate_top)
    ...
    integer :: k
    real(kind=DEFAULT_PRECISION) ::absorber_term
...
#endif
#ifdef W_ACTIVE
      if (anelastic_enabled .eqv. .true.) then
        absorber_term = 1.0 ! (tzc1(k))/5*(tzc1(k) + tzc2(k))
      else
        absorber_term = 1.0
      end if
      source_field%data(k, y_source_index, x_source_index)=source_field%data(k, y_source_index, x_source_index)+&
           4.0_DEFAULT_PRECISION*(w%data(k-1, y_flow_index, x_flow_index)* flux_z(k)*tzc1(k) - &
           w%data(k, y_flow_index, x_flow_index)*flux_z(k+1)*tzc2(k)) * absorber_term ! * (tzc1(k))/(tzc1(k) + tzc2(k)) stabilizer for high w speed in case of using anelastic approximation for density
#endif
#ifdef U_ACTIVE
      source_field%data(k, y_source_index, x_source_index)=source_field%data(k, y_source_index, x_source_index)+&
           (u%data(k, y_flow_index, x_flow_index-1)* flux_x(k) - u%data(k, y_flow_index, x_flow_index)*flux_x_previous(k))*&
  ...
  end subroutine differentiate_face_values_q
