...
contains
  ...
  subroutine initialisation_callback_viscosity(current_state)
    ...
    allocate(current_state%u_viscosity(z_size), current_state%v_viscosity(z_size), current_state%w_viscosity(z_size))
  ...
  subroutine finalisation_callback_viscosity(current_state)
    ...
    if (allocated(current_state%u_viscosity)) deallocate(current_state%u_viscosity)
    if (allocated(current_state%v_viscosity)) deallocate(current_state%v_viscosity)
    if (allocated(current_state%w_viscosity)) deallocate(current_state%w_viscosity)
  ...
  subroutine calculate_viscous_sources(current_state, local_y, local_x, tau12, tau12_ym1, tau12m1, &
    ...
    do k=2, current_state%local_grid%size(Z_INDEX)
#ifdef U_ACTIVE
      current_state%u_viscosity(k)=((tau11p1(k)-tau11(k))*current_state%global_grid%configuration%horizontal%cx+ &
           (tau12(k)-tau12_ym1(k))*&
      ...
      current_state%su%data(k, local_y, local_x)=current_state%su%data(k, local_y, local_x)+current_state%u_viscosity(k)
#endif
#ifdef V_ACTIVE
      current_state%v_viscosity(k)=((tau12(k)-tau12m1(k))*current_state%global_grid%configuration%horizontal%cx+ &
           (tau22_yp1(k)-tau22(k))*&
           current_state%global_grid%configuration%horizontal%cy+(tau23(k)-tau23(k-1))*&
           current_state%global_grid%configuration%vertical%rdz(k))/current_state%global_grid%configuration%vertical%rhon(k)
      current_state%sv%data(k, local_y, local_x)=current_state%sv%data(k, local_y, local_x)+current_state%v_viscosity(k)
#endif
    end do
#ifdef W_ACTIVE
    do k=2, current_state%local_grid%size(Z_INDEX)-1
      current_state%w_viscosity(k)=((tau13(k)-tau13m1(k))*current_state%global_grid%configuration%horizontal%cx+ &
           (tau23(k)-tau23_ym1(k))*&
      ...
      current_state%sw%data(k, local_y, local_x)=current_state%sw%data(k, local_y, local_x)+current_state%w_viscosity(k)
    end do
#endif
