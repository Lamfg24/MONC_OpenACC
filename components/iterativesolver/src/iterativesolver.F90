...
module iterativesolver_mod
  ...
  public initialisation_callback_iterativesolver, timestep_callback_iterativesolver, &
         finalisation_callback_iterativesolver
contains


  !> Initialisation callback hook which will set up the halo swapping state and allocate some data
  !! @param current_state The current model state
  subroutine initialisation_callback_iterativesolver(current_state)
    ...
    do iter = 1,current_state%config_args
      ...
      else if (current_state%options_database_string(iter,1) .eq. "tolerance") then
        read(current_state%options_database_string(iter,2),*) realnum
        tolerance = realnum
      else if (current_state%options_database_string(iter,1) .eq. "max_iterations") then
        read(current_state%options_database_string(iter,2),*) intnum
        max_iterations = intnum
      else if (current_state%options_database_string(iter,1) .eq. "preconditioner_iterations") then
        read(current_state%options_database_string(iter,2),*) intnum
        preconditioner_iterations = intnum
      else if (current_state%options_database_string(iter,1) .eq. "symm_prob") then
        read(current_state%options_database_string(iter,2),*) logicnum
        symm_prob = logicnum
      end if
    end do
  ...
  end subroutine initialisation_callback_iterativesolver

  !> Timestep callback, this ignores all but the last column where it calls the solver
  !! @param current_state The current model state
  subroutine timestep_callback_iterativesolver(current_state)
    type(model_state_type), target, intent(inout) :: current_state
