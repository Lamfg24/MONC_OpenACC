  ...
  subroutine timestep_callback_iobridge(current_state)
    ...
    if (current_state%parallel%my_global_rank == 1 .and. init_v .eq. 0) then
      ...
      call mpi_send(current_state%last_cfl_timestep, 1, MPI_INT, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%n_w_bins, 1, MPI_INT, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      ...
      call mpi_send(current_state%checkpoint_frequency, 1, MPI_INT, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%dim_var_to_mean, 1, MPI_INT, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      ...
      call mpi_send(current_state%diagnostic_file_3d_write_frequency, 1, MPI_INT, &
                current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%diagnostic_file_averaged_write_frequency, 1, MPI_INT, &
                current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
      ...
      call mpi_send(vertical_grid%zn, global_grid_z_size, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(vertical_grid%dz, global_grid_z_size, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(vertical_grid%rho, global_grid_z_size, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(vertical_grid%rhon, global_grid_z_size, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(vertical_grid%thref, global_grid_z_size, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(vertical_grid%rprefrcp, global_grid_z_size, MPI_DOUBLE, &
                    current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
      ...
      call mpi_send(current_state%socrates_enabled, 1, MPI_LOGICAL, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%flux_budget_enabled, 1, MPI_LOGICAL, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%pdf_analysis_enabled, 1, MPI_LOGICAL, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%diagnostics_averaged_enabled, 1, MPI_LOGICAL, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%checkpoint_path, 200, MPI_CHAR, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      call mpi_send(current_state%diagnostic_path, 200, MPI_CHAR, &
                    current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, ierr)
      init_v = init_v + 1
    end if
    ...
    if (current_state%modulo_number_averaged_write .eq. 0) then
      if (current_state%parallel%my_global_rank == 1) then
        if (current_state%diagnostics_averaged_enabled .eqv. .true.) then
          call mpi_send(current_state%mean_th, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_u, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_v, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qv, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_ql, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qr, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qi, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qs, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qg, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qAitkenSolMass, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qAccumSolMass, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qAccumInsolMass, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qCoarseSolMass, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_qCoarseDustMass, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nl, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nr, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_ni, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_ns, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_ng, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nAitkenSolNumber, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nAccumSolNumber, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nAccumInsolNumber, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nCoarseSolNumber, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_nCoarseDustnumber, (global_grid_z_size), &
                              MPI_DOUBLE, current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)

          call mpi_send(current_state%mean_mo_l, 1, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process,&
                      1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_friction_vel, 1, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process,&
                      1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_z0, 1, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process,&
                      1000, MPI_COMM_WORLD, ierr)
          call mpi_send(current_state%mean_z0th, 1, MPI_DOUBLE, current_state%parallel%corresponding_io_server_process,&
                      1000, MPI_COMM_WORLD, ierr)
        end if
      end if
    end if
...
!
  ...
  subroutine diagnostic_file_0d_write(current_state, vertical_grid, global_array_size, &
    ...
    if (current_state%parallel%my_global_rank == 1) then
      call mpi_recv(current_state%diag_0d_done, 1, MPI_LOGICAL, current_state%parallel%corresponding_io_server_process, &
  ...
  subroutine diagnostic_file_1d_write(current_state, vertical_grid, global_array_size, &
    ...
    if (current_state%flux_budget_enabled .eqv. .true.) then
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_tendency)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qv_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ql_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qr_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qi_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qs_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qg_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAitkenSolMass_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumSolMass_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumInsolMass_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseSolMass_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseDustMass_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nl_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nr_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ni_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ns_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ng_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAitkenSolNumber_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumSolNumber_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumInsolNumber_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseSolNumber_flux_values)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseDustnumber_flux_values)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qv_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ql_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qr_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qi_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qs_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qg_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAitkenSolMass_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumSolMass_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumInsolMass_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseSolMass_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseDustMass_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nl_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nr_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ni_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ns_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ng_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAitkenSolNumber_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumSolNumber_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumInsolNumber_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseSolNumber_gradient)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseDustnumber_gradient)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qv_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ql_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qr_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qi_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qs_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qg_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAitkenSolMass_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumSolMass_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumInsolMass_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseSolMass_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseDustMass_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nl_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nr_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ni_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ns_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ng_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAitkenSolNumber_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumSolNumber_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumInsolNumber_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseSolNumber_diff)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseDustnumber_diff)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qv_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ql_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qr_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qi_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qs_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qg_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAitkenSolMass_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumSolMass_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumInsolMass_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseSolMass_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseDustMass_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nl_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nr_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ni_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ns_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ng_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAitkenSolNumber_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumSolNumber_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumInsolNumber_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseSolNumber_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseDustnumber_buoyancy)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qv_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ql_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qr_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qi_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qs_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qg_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAitkenSolMass_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumSolMass_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qAccumInsolMass_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseSolMass_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qCoarseDustMass_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nl_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nr_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ni_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ns_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ng_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAitkenSolNumber_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumSolNumber_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nAccumInsolNumber_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseSolNumber_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%nCoarseDustnumber_tendency)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uw_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vw_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uw_viscosity)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vw_viscosity)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uw_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vw_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uw_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vw_tendency)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uw_w)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vw_w)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tu_su)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uu_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%uu_viscosity)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wu_u)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tv_sv)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vv_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vv_viscosity)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wv_v)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tw_sw)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ww_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ww_viscosity)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ww_buoyancy)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%u_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%us_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%u_thetal_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%u_thetal_viscosity_diffusion)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wu_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%v_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%vs_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%v_thetal_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%v_thetal_viscosity_diffusion)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wv_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ws_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_thetal_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_thetal_viscosity_diffusion)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_thetal_buoyancy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%ww_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%thetal_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%sthetal_thetal)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%thetal_thetal_advection)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%thetal_thetal_diffusion)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wthetal_thetal)

      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%shprd)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_ke)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_p)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%sub_buoy)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tke_tend)
    end if

    if (current_state%conditional_diagnostics_column_enabled .eqv. .true.) then
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_zn_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_zn2_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tmp_th_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wth_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wth_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%thv_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wthv_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_pr2_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wthsg_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_zn3_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%relhum_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tmp_u_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%tmp_v_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wu_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wv_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wusg_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wvsg_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%TdegK_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_h_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_h_pr1_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%th_h_pr2_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qvli_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qvli_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qvli_pr2_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qppt_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qppt_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%qppt_pr2_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wqvli_pr_cln)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%wqppt_pr_cln)
    end if

    if (current_state%pdf_analysis_enabled .eqv. .true.) then
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%global_grid%configuration%vertical%w_dwn)
      call gather1D_and_send_to_IO(current_state, global_grid_z_size, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%global_grid%configuration%vertical%w_up)
      call gather1D_bin_and_send_to_IO(current_state, global_grid_z_size, current_state%n_w_bins, &
                                    global_array_size, local_grid_z_size, &
                                    global_grid_z_start, current_state%w_histogram_profile_local)
    end if

    if (current_state%parallel%my_global_rank == 1) then
      call mpi_recv(current_state%diag_1d_done, 1, MPI_LOGICAL, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
  ...
  subroutine diagnostic_file_2d_write(current_state, vertical_grid, global_array_size, &
    ...
    if (current_state%scalar_diagnostics_enabled .eqv. .true.) then
      ...
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                  global_array_size, local_grid_y_size, local_grid_x_size, &
                                  global_grid_y_start, global_grid_x_start, current_state%vwp_usr)
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                  global_array_size, local_grid_y_size, local_grid_x_size, &
                                  global_grid_y_start, global_grid_x_start, current_state%lwp)
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                  global_array_size, local_grid_y_size, local_grid_x_size, &
                                  global_grid_y_start, global_grid_x_start, current_state%lwp_usr)
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                  global_array_size, local_grid_y_size, local_grid_x_size, &
                                  global_grid_y_start, global_grid_x_start, current_state%wmax)
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
      ...
      if ((current_state%casim_enabled .eqv. .true.) .or. (current_state%simplecloud_enabled .eqv. .true.)) then
        ...
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%rwp_usr)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%iwp)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%iwp_usr)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%swp)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%swp_usr)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%gwp)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%gwp_usr)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%tot_iwp)
        call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                    global_array_size, local_grid_y_size, local_grid_x_size, &
                                    global_grid_y_start, global_grid_x_start, current_state%tot_iwp_usr)
      end if
      call gather2D_and_send_to_IO(current_state, global_grid_y_size, global_grid_x_size, &
                                  global_array_size, local_grid_y_size, local_grid_x_size, &
                                  global_grid_y_start, global_grid_x_start, current_state%subke_2d)
    ...
    if (current_state%parallel%my_global_rank == 1) then
      call mpi_recv(current_state%diag_2d_done, 1, MPI_LOGICAL, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
  ...
  subroutine diagnostic_file_3d_write(current_state, vertical_grid, global_array_size, &
    ...
    if (current_state%parallel%my_global_rank == 1) then
      call mpi_recv(current_state%diag_3d_done, 1, MPI_LOGICAL, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
  ...
  subroutine send_data_for_checkpoint(current_state, vertical_grid, global_array_size, &
    ...
    if (current_state%parallel%my_global_rank == 1) then
      call mpi_recv(current_state%checkpt_done, 1, MPI_LOGICAL, current_state%parallel%corresponding_io_server_process, &
                    1000, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
  ...
  subroutine gather1D_bin_and_send_to_IO(current_state, global_grid_z_size, n_w_bins, &
                                   global_array_size, local_grid_z_size, &
                                   global_grid_z_start, data_1D_bin)


    type(model_state_type), target, intent(inout) :: current_state
    integer, intent(in) :: global_grid_z_size, n_w_bins, global_array_size, &
                           local_grid_z_size, global_grid_z_start
    real(kind=DEFAULT_PRECISION), dimension(:,:), intent(in) :: data_1D_bin
    real(kind=DEFAULT_PRECISION), dimension(:,:), allocatable :: global_array_1d_bin
    integer :: iter_bin, iter_z, ierr

    allocate(global_array_1d_bin(global_grid_z_size, n_w_bins))
    global_array_1d_bin = 0.0_DEFAULT_PRECISION

    do iter_bin = 1, n_w_bins
      do iter_z = 1, local_grid_z_size
        global_array_1d_bin(global_grid_z_start+ iter_z -1, iter_bin) = &
                        data_1D_bin(iter_z, iter_bin)
      end do
    end do

    if (current_state%parallel%my_rank == 0) then
        call mpi_reduce(MPI_IN_PLACE , global_array_1d_bin, global_array_size*n_w_bins, &
                      PRECISION_TYPE, MPI_SUM, 0, current_state%parallel%monc_communicator, ierr)
    else
      call mpi_reduce(global_array_1d_bin, global_array_1d_bin, global_array_size*n_w_bins, &
                      PRECISION_TYPE, MPI_SUM, 0, current_state%parallel%monc_communicator, ierr)
    end if
    if (current_state%parallel%my_global_rank == 1) then
      call mpi_send(global_array_1d_bin, global_array_size*n_w_bins*2, MPI_REAL, &
                    current_state%parallel%corresponding_io_server_process, 1000, MPI_COMM_WORLD, ierr)
      deallocate(global_array_1d_bin)
    end if
  end subroutine gather1D_bin_and_send_to_IO
