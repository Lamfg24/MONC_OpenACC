...
contains
  ...
  subroutine initialisation_callback_buoyancy(current_state)
    ...
    allocate(current_state%w_buoyancy(z_size))

    current_state%w_buoyancy = 0.0_DEFAULT_PRECISION
  ...
  subroutine finalisation_callback_buoyancy(current_state)
    ...
    if (allocated(current_state%w_buoyancy)) deallocate(current_state%w_buoyancy)
      ...
      if (current_state%immersed%ib_enabled .eqv. .true.) then
        ...
        do k=2,current_state%local_grid%size(Z_INDEX)-1
          ...
          current_state%sw%data(k, current_state%column_local_y, current_state%column_local_x)=&
              current_state%sw%data(k, current_state%column_local_y, current_state%column_local_x)+current_state%w_buoyancy(k)
        end do
      end if
    end if
