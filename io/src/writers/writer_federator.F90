...
module writer_federator_mod
  ...
  use mpi, only : MPI_INT, MPI_MAX, MPI_COMM_WORLD, MPI_STATUSES_IGNORE, MPI_BYTE, MPI_STATUS_IGNORE, MPI_COMM_RANK, &
...
contains
  ...
  subroutine inform_writer_federator_time_point(io_configuration, source, data_id, data_dump)
    ...
    call check_write_criteria(io_configuration, real(time, kind=4), timestep, terminated, data_id)

  end subroutine inform_writer_federator_time_point


  subroutine check_write_criteria(io_configuration, time, timestep, terminated, data_id)
    type(io_configuration_type), intent(inout) :: io_configuration
    real, intent(in) :: time
    integer, intent(in) :: timestep, data_id
    ...
    do i=1,size(writer_entries)
      !print*,"check_write_criteria command = ",command
      !print*,"check_write_criteria index_writer_entries = ",i
      issue_write = .false.
      ...
      !print*,"check_write_criteria writer_entries(i)%latest_pending_write_timestep = ", &
      !        writer_entries(i)%latest_pending_write_timestep
      !print*,"check_write_criteria timestep = ", timestep

      if (writer_entries(i)%latest_pending_write_timestep .ne. timestep) then ! ok
        issue_terminated_write=writer_entries(i)%write_on_terminate .and. terminated
        !******************** MODIF ******************
        if (command .eq. 20) then
          if (i .eq. 1) then
            issue_write = .true.
          else
            issue_write = .false.
        ...
        else if (command .ge. 6 .or. command .le. 19) then
          issue_write = .false.
        end if
        !******************** MODIF ******************
        !******************** ORIGINAL ******************
        !
        !print*,"check_write_criteria writer_entries(i)%write_on_model_time = ", writer_entries(i)%write_on_model_time
        !print*,"check_write_criteria writer_entries(i)%write_time_frequency = ", writer_entries(i)%write_time_frequency
        !if (writer_entries(i)%write_on_model_time) then
        !  if (writer_entries(i)%write_time_frequency .gt. 0) then
        ...
        !    print*,"check_write_criteria issue_writeaaa = ",issue_write
        !  end if
        !else
        !  ! Case for all checkpoint requests
        !  print*,"check_write_criteria issue_writebbb = ",issue_write
        !  if (writer_entries(i)%write_timestep_frequency .gt. 0) then
        !    print*,"check_write_criteria issue_writebbb1 = ",issue_write
        !    issue_write = mod(timestep, writer_entries(i)%write_timestep_frequency) == 0
        !    print*,"check_write_criteria issue_writebbb2 = ",issue_write
        !  end if
        !end if
        !******************** ORIGINAL ******************
      ...
      !print*,"check_write_criteria issue_write = ",issue_write
      !print*,"check_write_criteria issue_terminated_write = ",issue_terminated_write
      if (issue_write .or. issue_terminated_write) then
        !print*,"check_write_criteria index_writer_entriesaaaa = ",i
  ...
  logical function is_field_used_by_writer_federator(field_name, field_namespace)
    ...
    if (c_contains(used_field_names, field_name)) then
      !print*,"is_field_used_by_writer_federator0000"
  ...
  subroutine enable_specific_field_by_name(field_name, diagnostics_mode, expected_here)
    ...
    do while (continue_search)
      ...
      !print*,"enable_specific_field_by_name0000"
  ...
  subroutine provide_q_field_names_to_writer_federator(q_provided_field_names)
    ...
    do while (c_has_next(iterator))
      ...
      do while (c_has_next(q_field_iterator))
        ...
        do while (continue_search)
          contents_index=contents_index+1
          !print*,"provide_q_field_names_to_writer_federator00000"
  ...
  subroutine provide_tracer_names_to_writer_federator(tracer_provided_field_names)
    ...
    do while (c_has_next(iterator))
      ...
      do while (c_has_next(tracer_iterator))
        ...
        do while (continue_search)
          contents_index=contents_index+1
          print*,"provide_tracer_names_to_writer_federator0000"
          continue_search=get_next_applicable_writer_entry(search_field, writer_index_point=writer_index, &
               contents_index_point=contents_index)
          if (continue_search) then
            writer_entries(writer_index)%contents(contents_index)%field_name=field_name
  ...
  subroutine provide_ordered_field_to_writer_federator(io_configuration, field_name, field_namespace, field_values, &
    ...
    if (field_values%data_type == DOUBLE_DATA_TYPE) then
      print*,"provide_ordered_field_to_writer_federatoraaaa"
      ...
      print*,"provide_ordered_field_to_writer_federatorbbbb"
      continue_search=.true.
      writer_index=1
      contents_index=0
      ...
      if (c_contains(used_field_names, field_name)) then
        do while (continue_search)
          ...
          if (continue_search) then
            !print*,"provide_ordered_field_to_writer_federatorbbbb and writer_index = ", writer_index
            !print*,"provide_ordered_field_to_writer_federatorbbbb and contents_index = ", contents_index
            ...
            !print*,"writer_entries(writer_index)%contents(contents_index)%pending_to_write = ", &
            ...
            if (writer_entries(writer_index)%contents(contents_index)%pending_to_write) then
              print*,"provide_ordered_field_to_writer_federator determine_if_outstanding"
  ...
  subroutine provide_ordered_field_to_writer_federator_real_values(io_configuration, field_name, field_namespace, field_values, &
    ...
    if (c_contains(used_field_names, field_name)) then
      !print*,"provide_ordered_field_to_writer_federator_real_valuesaaaaa"
      !print*,"provide_ordered_field_to_writer_federator_real_values field_name = ",field_name
      !print*,"provide_ordered_field_to_writer_federator_real_values field_namespace = ",field_namespace
      ...
      index=1
      !print*,"provide_ordered_field_to_writer_federator_real_valuesbbbb"
      do while (c_has_next(iterator))
        !print*,"provide_ordered_field_to_writer_federator_real_values field_name = ",field_name
        !print*,"provide_ordered_field_to_writer_federator_real_values c_next_string(iterator) = ",c_next_string(iterator)
      ...
      !print*,"provide_ordered_field_to_writer_federator_real_valuesccccc" !! jamais utilisé
    end if
  ...
  subroutine provide_ordered_single_field_to_writer_federator(io_configuration, field_name, field_namespace, field_values, &
    ...
    print*,"provide_ordered_single_field_to_writer_federator00000"
    continue_search=.true.
    writer_index=1
    contents_index=0
    if (c_contains(used_field_names, field_name)) then
      do while (continue_search)
        contents_index=contents_index+1
        !print*,"provide_ordered_single_field_to_writer_federator writer_index = ",writer_index
        !print*,"provide_ordered_single_field_to_writer_federator contents_index = ",contents_index
        continue_search=get_next_applicable_writer_entry(field_name, field_namespace, writer_index, contents_index)
        !print*,"provide_ordered_single_field_to_writer_federator writer_index = ",writer_index
        !print*,"provide_ordered_single_field_to_writer_federator contents_index = ",contents_index
        if (continue_search) then
          ...
          print*,"provide_ordered_single_field_to_writer_federator111111"
          writer_entries(writer_index)%contents(contents_index)%enabled=.true.
          if (.not. c_contains(typed_result_values, conv_to_string(&
            ...
            print*,"provide_ordered_single_field_to_writer_federator22222aaaaa"
            if (writer_entries(writer_index)%contents(contents_index)%collective_write .and. source .gt. -1) then
              print*,"provide_ordered_single_field_to_writer_federatoraaaa"
              ...
              print*,"provide_ordered_single_field_to_writer_federatorbbbb"
              print*,"writer_entries(writer_index)%contents(contents_index)%output_frequency = ", &
                      writer_entries(writer_index)%contents(contents_index)%output_frequency
              result_values=writer_entries(writer_index)%contents(contents_index)%time_manipulation(field_values, &
                   writer_entries(writer_index)%contents(contents_index)%output_frequency, &
            ...
            print*,"provide_ordered_single_field_to_writer_federator22222bbbbb"
          ...
          if (allocated(result_values%values)) then
            print*,"provide_ordered_single_field_to_writer_federator3333"
            print*,"provide_ordered_single_field_to_writer_federator source = ",source
            writer_entries(writer_index)%contents(contents_index)%latest_timestep_values=timestep
            if (log_get_logging_level() .ge. LOG_DEBUG) then
            ...
            !print*,"posftwf .not. c_is_empty(writer_entries(writer_index)%contents(contents_index)%values_to_write) = ", &
            !        .not. c_is_empty(writer_entries(writer_index)%contents(contents_index)%values_to_write)
            call check_thread_status(forthread_mutex_unlock(writer_entries(writer_index)%contents(contents_index)%values_mutex))
            !print*,"posftwf writer_entries(writer_index)%contents(contents_index)%pending_to_write = ", &
            !        writer_entries(writer_index)%contents(contents_index)%pending_to_write
            print*,"provide_ordered writer_entries(writer_index)%contents(contents_index)%pending_to_write = ", &
                   writer_entries(writer_index)%contents(contents_index)%pending_to_write
            if (writer_entries(writer_index)%contents(contents_index)%pending_to_write) then !!!!! VRAIMENT UN PROBLEME POUR LE PARALLELISME
              print*,"provide_ordered_single_field_to_writer_federator and determine_if_outstanding"
  ...
  subroutine determine_if_outstanding_field_can_be_written(io_configuration, writer_entry, specific_field)
    ...
    integer ::  tag_close_diag = 919, ierr

    print*,"determine_if_outstanding_field_can_be_written0000000000"
    if (specific_field%pending_to_write) then    
      ...
      print*,"determine_if_outstanding_field_can_be_written field_write_success = ",field_write_success
      if (field_write_success) then
        ...
        if (do_close_num_fields) then
          print*,"determine_if_outstanding_field_can_be_written_close_diagnostics_file"
          call close_diagnostics_file(io_configuration, writer_entry, writer_entry%write_timestep, writer_entry%write_time)
          ! LAMBERT permet d'écrire la suite des fichiers diag commande 19 et ferme surtout le fichier diag_3d
          call mpi_send(tag_close_diag, 1, MPI_INT, 0, 919, MPI_COMM_WORLD, ierr) !********** MODIF ***********
        end if    
      end if
    end if
    print*,"determine_if_outstanding_field_can_be_written1111111111111111111"
  ...
  subroutine determine_if_field_can_be_written(io_configuration, writer_entry, specific_field, &
    ...
    print*,"determine_if_field_can_be_written000000"
    num_matching=0
    largest_value_found=0.0
    call check_thread_status(forthread_mutex_lock(specific_field%values_mutex))
    print*,"specific_field%field_name = ",specific_field%field_name
    print*,"specific_field%field_namespace = ",specific_field%field_namespace
    print*,".not. c_is_empty(specific_field%values_to_write) = ", &
              .not. c_is_empty(specific_field%values_to_write)
    if (.not. c_is_empty(specific_field%values_to_write)) then ! doit être plein
      ...
      do while (c_has_next(iterator))
        ...
        print*,"value_to_test = ",value_to_test
        print*,"write_time = ",write_time
        print*,"previous_write_time = ",previous_write_time
...
!
    ...
    print*,"timeseries_diag%num_entries = ",timeseries_diag%num_entries
    print*,"specific_field%collective_write = ",specific_field%collective_write
    print*,"specific_field%collective_contiguous_optimisation = ",specific_field%collective_contiguous_optimisation
    !if ((num_matching .eq. timeseries_diag%num_entries)) then ! original
    ...
    print*,"specific_field%pending_to_write = ",specific_field%pending_to_write
    print*,"determine_if_field_can_be_written111111"
  ...
  subroutine check_writer_for_trigger(io_configuration, source, data_id, data_dump)
    ...
    if (is_field_present(io_configuration, source, data_id, "timestep") .and. &
      ...
      command = data_id + 4 !**** MODIF ***

      !***** MODIF *****
      !do i=1,size(writer_entries)
        !print*,".not. c_is_empty(writer_entries(i)%pending_writes)_check =", &
        !          .not. c_is_empty(writer_entries(i)%pending_writes)
        !do j=1, size(writer_entries(i)%contents)
          !print*,"check_writer_for_trigger writer_entries(i)%contents(j)%field_name = ", &
          !      writer_entries(i)%contents(j)%field_name
          !print*,"check_writer_for_trigger writer_entries(i)%contents(j)%field_namespace = ", &
          !      writer_entries(i)%contents(j)%field_namespace
        !print*,"check_writer_for_trigger writer_entries(i)%contents(j)%pending_to_write = ", &
          !      writer_entries(i)%contents(j)%pending_to_write
          !print*,"check_writer_for_trigger .not. c_is_empty(writer_entries(i)%contents(j)%values_to_write) = ", &
          !    .not. c_is_empty(writer_entries(i)%contents(j)%values_to_write)
          !if (command .eq. 20) then
          !  writer_entries(i)%contents(j)%pending_to_write = .false.
          !end if
       ! end do
      !end do
      !***** MODIF *****
      ...
      !call check_thread_status(forthread_mutex_lock(currently_writing_mutex)) ! LAMBERT essai
      !if (.not. currently_writing) then
      !  print*,"check_writer_for_trigge-ccc"
      !  call check_thread_status(forthread_mutex_unlock(currently_writing_mutex))
      !  call evaluate_writer_issue(io_configuration, data_id) !! ecrit les données
      !else
      !  print*,"check_writer_for_trigge-ddd"
      !  call check_thread_status(forthread_mutex_unlock(currently_writing_mutex))
      !end if  ! LAMBERT essai
      call evaluate_writer_issue(io_configuration, data_id) !! MODIF ecrit les données
      !********** MODIF ***********
    end if
  ...
  subroutine evaluate_writer_issue(io_configuration, data_id, chain_override)
    type(io_configuration_type), intent(inout) :: io_configuration
    logical, intent(in), optional :: chain_override
    integer, intent(in) :: data_id
  
    integer :: timestep, index_to_issue, command !**** MODIF ***
    logical :: terminated, regular_pending_exists, non_io_dump_pending_exists, &
               non_io_regular_exists, override, first_instance
    logical, dimension(size(writer_entries)) :: non_io_dump_pending_exists_array!**** MODIF ***
    type(iterator_type) :: iterator
    type(mapentry_type) :: map_entry
    integer :: i, j, next_pending_timestep
    ...
    ! Lock activity on critical items
    print*,"ewi000000 command = ",command
    call check_thread_status(forthread_mutex_lock(currently_writing_mutex))
    do i=1,size(writer_entries)
      !print*,"ewi command = ",command
      !print*,"ewi size(writer_entries(i)%contents) = ",size(writer_entries(i)%contents)
      do j=1, size(writer_entries(i)%contents)
        !print*,"ewi evaluate_writer_issue_writer_index_i = ",i
        !print*,"ewi evaluate_writer_issue_content_index_j = ",j
        !print*,"ewi writer_entries(i)%contents(j)%field_name = ", &
        !        writer_entries(i)%contents(j)%field_name
        !print*,"ewi writer_entries(i)%contents(j)%field_namespace = ", &
        !        writer_entries(i)%contents(j)%field_namespace
        !print*,"ewi writer_entries(i)%contents(j)%pending_to_write = ", &
        !        writer_entries(i)%contents(j)%pending_to_write
        !print*,"ewi .not. c_is_empty(writer_entries(i)%contents(j)%values_to_write) = ", &
        !      .not. c_is_empty(writer_entries(i)%contents(j)%values_to_write)
      end do
      call check_thread_status(forthread_mutex_lock(writer_entries(i)%pending_writes_mutex))
    end do
    print*,"ewi1111111"
    ! Only look for files to write if no files are being written.
    print*,"(.not. currently_writing) = ",(.not. currently_writing)
    print*,"override = ",override
    if ((.not. currently_writing) .or. override) then
      print*,"ewi1111111aaaaaaa"
      ! Find next pending write timestep
      do i=1,size(writer_entries)
        print*,".not. c_is_empty(writer_entries(i)%pending_writes)00 =", &
                  .not. c_is_empty(writer_entries(i)%pending_writes)
        !print*,"evaluate_writer_issue_i = ",i
        !print*,"size(writer_entries(i)%contents) = ",size(writer_entries(i)%contents)
        !do j=1, size(writer_entries(i)%contents)
        !print*,".not. c_is_empty(writer_entries(i)%contents(j)%values_to_write)111 = ", &
        !          .not. c_is_empty(writer_entries(i)%contents(j)%values_to_write)
        !end do
        print*,"ewi1111111aaaaaa000000"
        if (.not. c_is_empty(writer_entries(i)%pending_writes)) then
          print*,"ewi1111111aaaaaa111111"
          generic=>c_get_generic(writer_entries(i)%pending_writes,1)
          print*,"ewi1111111aaaaaa222222"
          select type(generic)
          type is (pending_write_type)
          print*,"next_pending_timestep = ",next_pending_timestep
          print*,"generic%timestep = ",generic%timestep
          ...
          !************* MODIF ***********
          if (command .eq. 5 .or. command .eq. 19) then
            !print*,"evaluate_writer_issue1 and i = ", i
            if (i .ne. 1 ) then
              writer_entries(i)%contains_io_status_dump = .true.
              non_io_dump_pending_exists_array(i) = non_io_dump_pending_exists
                if (.not. writer_entries(i)%write_on_terminate) then
                  non_io_regular_exists = .true.
                end if
            else
              writer_entries(i)%contains_io_status_dump = .false.
              non_io_dump_pending_exists_array(i) = .false.
            end if
          else ! LAMBERT modif
            non_io_dump_pending_exists_array(i) = non_io_dump_pending_exists ! LAMBERT modif
          end if
      ...
      if (next_pending_timestep .lt. huge(next_pending_timestep)) then
        ...
        do i=1,size(writer_entries)
          ...
          if (.not. c_is_empty(writer_entries(i)%pending_writes)) then
            ...
            print*,"writer_entries(i)%contains_io_status_dump = ", writer_entries(i)%contains_io_status_dump
            print*,"writer_entries(i)%contains_io_status_dump = ", &
                    writer_entries(i)%contains_io_status_dump
            print*,"non_io_dump_pending_exists = ", non_io_dump_pending_exists
            ...
            print*,"ewi timestep_evaluater = ",timestep
            print*,"ewi next_pending_timestep_evaluater = ",next_pending_timestep
            print*,"ewi first_instance_evaluater = ",first_instance
            print*,"ewi i33333 = ",i
            if (timestep .eq. next_pending_timestep .and. first_instance) then
              ...
              !print*,"ewi issue_actual_write_index_to_issue = ",index_to_issue
      ...
      if (index_to_issue .gt. 0) then
        ...
        select type(generic)
          ...
          !print*,"ewi evaluate_writer_issue and issue_actual_write"
          !print*,"ewi size(writer_entries(index_to_issue)%contents) = ", size(writer_entries(index_to_issue)%contents)
          !print*,"ewi issue_actual_write index_to_issue = ",index_to_issue
          !print*,"ewi issue_actual_write_command = ", command
          call issue_actual_write(io_configuration, writer_entries(index_to_issue), &  ! writer_entries(index_to_issue) = writer_entry
  ...
  subroutine issue_actual_write(io_configuration, writer_entry, timestep, time, terminated_write)
    ...
    num_written=0
    call check_thread_status(forthread_mutex_lock(writer_entry%num_fields_to_write_mutex))
    print*,"issue_actual_write22222"
    print*,"size(writer_entry%contents) = ",size(writer_entry%contents)
    do j=1, size(writer_entry%contents)  
      if (writer_entry%contents(j)%enabled .and. writer_entry%contents(j)%expected_here) then
        ...
        !print*,"issue_actual_write contents_index = ",j
        !print*,"issue_actual_write determine_if_field_can_be_written"
        !print*,"issue_actual_write writer_entry%contents(j)%field_namespace = ",writer_entry%contents(j)%field_namespace

        call determine_if_field_can_be_written(io_configuration, writer_entry, writer_entry%contents(j), timestep, & ! writer_entry%contents(j) = specific_field
    ...
    !print*,"issue_actual_write44444"
    print*,"issue_actual_write total_outstanding = ",total_outstanding
    if (total_outstanding == 0) then
      print*,"issue_actual_write_close_diagnostics_file"
      call close_diagnostics_file(io_configuration, writer_entry, timestep, time)
    end if
    print*,"issue_actual_write5555@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
  ...
  subroutine close_diagnostics_file(io_configuration, writer_entry, timestep, time)
    ...
    print*,"close_diagnostics_file00000"
    print*,"writer_entry%filename = ",writer_entry%filename
  ...
  subroutine handle_close_diagnostics_globalcallback(io_configuration, values, filename, timestep)
    ...
    integer :: timestep, command
    integer :: ierr, tag_close_diag, tag_close_check

    type(writer_type), pointer :: writer_entry
    integer :: i
    logical :: terminated, ready

    writer_entry=>get_writer_entry_from_netcdf(filename, timestep, terminated)
    print*,"handle_close_diagnostics_globalcallback0000"
    do i=1, size(writer_entry%contents)
    ...
    print*,"handle_close_diagnostics_globalcallbackaaaaa"
    ...
    print*,"handle_close_diagnostics_globalcallbackbbbbb"
    call clean_time_points()
    print*,"handle_close_diagnostics_globalcallbackcccccc"

    !*************** ecriture des variables serialised ***************
    if (writer_entry%contains_io_status_dump) then
      print*,"handle_close_diagnostics_globalcallbackeeee"
      if (.not. terminated) then
        call is_io_server_state_writer_ready(timestep, ready)
        print*,"ready = ",ready
    ...
    print*,"handle_close_diagnostics_globalcallback filename = ",filename
    print*,"handle_close_diagnostics_globalcallback11111"
    ...
    if (filename(1:16) .eq. "diagnostic_files") then
      command = 15
      call evaluate_writer_issue(io_configuration, command, chain_override=.true.) ! important car génére les prochaines écritures de fichiers diag
    else if (filename(1:16) .eq. "checkpoint_files") then
      command = 16
      call evaluate_writer_issue(io_configuration, command, chain_override=.true.) ! important car génére les prochaines écritures de fichiers diag
    end if

    if (filename(1:16) .eq. "diagnostic_files") then
      if (filename(28:29) .eq. "2d") then ! premier après diag_3d
        print*,"2d"
        tag_close_diag = 9191
        call mpi_send(tag_close_diag, 1, MPI_INT, 0, 9191, MPI_COMM_WORLD, ierr)
      else if (filename(28:29) .eq. "1d") then
        print*,"1d"
        tag_close_diag = 9192
        call mpi_send(tag_close_diag, 1, MPI_INT, 0, 9192, MPI_COMM_WORLD, ierr)
      else if (filename(28:29) .eq. "0d") then
        print*,"0d"
        tag_close_diag = 9193
        call mpi_send(tag_close_diag, 1, MPI_INT, 0, 9193, MPI_COMM_WORLD, ierr)
      else if (filename(28:29) .eq. "sd") then
        print*,"sd"
        tag_close_diag = 9194
        call mpi_send(tag_close_diag, 1, MPI_INT, 0, 9194, MPI_COMM_WORLD, ierr)
      else if (filename(28:29) .eq. "cd") then
        print*,"cd"
        tag_close_diag = 9195
        call mpi_send(tag_close_diag, 1, MPI_INT, 0, 9195, MPI_COMM_WORLD, ierr)

      end if
    else if (filename(1:16) .eq. "checkpoint_files") then
      tag_close_check = 200
      call mpi_send(tag_close_check, 1, MPI_INT, 0, 200, MPI_COMM_WORLD, ierr)
    end if
    print*,"handle_close_diagnostics_globalcallback1111"
  ...
  subroutine register_pending_file_write(writer_entry_index, timestep, time, terminated_write)
    ...
    print*,".not. c_is_empty(writer_entries(writer_entry_index)%pending_writes)_register_pending_file_write0 = ", &
            .not. c_is_empty(writer_entries(writer_entry_index)%pending_writes)
    call c_push_generic(writer_entries(writer_entry_index)%pending_writes, generic, .false.)
    print*,".not. c_is_empty(writer_entries(writer_entry_index)%pending_writes)_register_pending_file_write1 = ", &
            .not. c_is_empty(writer_entries(writer_entry_index)%pending_writes)
      ...
      do i=writer_index_point, size(writer_entries)
        if (contents_index_point .le. size(writer_entries(i)%contents)) then
          do j=contents_index_point, size(writer_entries(i)%contents)
            !print*,"writer_entries(i)%contents(j)%field_name = ",writer_entries(i)%contents(j)%field_name
            !print*,"writer_entries(i)%contents(j)%field_namespace = ",writer_entries(i)%contents(j)%field_namespace
            !counter = counter + 1
            !print*,"field_name = ",field_name
            if (writer_entries(i)%contents(j)%field_name==field_name) then
              !print*,"field_namespace = ",field_namespace
              !print*,"present(field_namespace) = ",present(field_namespace)
              if (present(field_namespace)) then                
                if (writer_entries(i)%contents(j)%field_namespace .ne. field_namespace) cycle
              end if
              !print*,"get_next_applicable_writer_entry field_name = ",field_name
              !print*,"get_next_applicable_writer_entry field_namespace = ",field_namespace
              writer_index_point=i
              contents_index_point=j
              get_next_applicable_writer_entry=.true.
              !print*,"get_next_applicable_writer_entry writer_index_point = ",writer_index_point
              !print*,"get_next_applicable_writer_entry contents_index_point = ",contents_index_point
              return
            end if
          end do
        end if
  ...
  integer function add_group_of_fields_to_writer_entry(io_configuration, writer_entry_index, facet_index, current_field_index, &
    ...
    !print*,"io_configuration%groups(group_index)%namespace0 = ", io_configuration%groups(group_index)%namespace
    ...
    do while (c_has_next(iterator))
      field_name=c_next_string(iterator)
      !print*,"io_configuration%groups(group_index)%namespace1 = ", io_configuration%groups(group_index)%namespace
  ...
  integer function add_field_to_writer_entry(io_configuration, writer_entry_index, io_config_facet_index, &
    ...
    !print*,"add_field_to_writer_entry field_namespace = ",field_namespace
    if (get_diagnostic_field_configuration(io_configuration, field_name, field_namespace, diagnostic_field_configuration)) then
      if (diagnostic_field_configuration%field_type == ARRAY_FIELD_TYPE) then
        if (diagnostic_field_configuration%dim_size_defns(diagnostic_field_configuration%dimensions) .eq. "qfields") then  
          number_q_fields=c_get_integer(io_configuration%dimension_sizing, "qfields")
  ...
  subroutine add_specific_field_to_writer_entry(io_configuration, writer_entry_index, io_config_facet_index, &
    ...
    !print*,"writer_entries(writer_entry_index)%contents(my_facet_index)%field_name = ", &
    !        writer_entries(writer_entry_index)%contents(my_facet_index)%field_name

    !print*,"writer_entries(writer_entry_index)%contents(my_facet_index)%field_namespace = ", &
    !        writer_entries(writer_entry_index)%contents(my_facet_index)%field_namespace
