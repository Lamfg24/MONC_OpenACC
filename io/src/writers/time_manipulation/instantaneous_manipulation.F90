...
contains
  ...
  type(data_values_type) function perform_instantaneous_time_manipulation(instant_values, output_frequency, &
    ...
    print*,"perform_instantaneous_time_manipulation0000"
    print*,"perform_instantaneous_time_manipulation output_frequency = ",output_frequency
    if (deduce_whether_to_issue_values(field_name, output_frequency, time, time_basis, timestep)) then
  ...
  logical function deduce_whether_to_issue_values(field_name, output_frequency, time, time_basis, timestep)
    character(len=*), intent(in) :: field_name
    real, intent(in) :: output_frequency
    real(kind=DEFAULT_PRECISION), intent(in) :: time
    logical, intent(in) :: time_basis
    ...
    real :: previous_time_write, time_difference, modulo_result
    logical :: select_value
    integer :: frequency

    print*,"deduce_whether_to_issue_values0000"
    ...
    print*,"deduce_whether_to_issue_values111111"
    ...
    !****** ORIGINAL ******
    !****** MODIF ******
    print*,"deduce_whether_to_issue_values nint(time) = ",nint(time)
    print*,"deduce_whether_to_issue_values time_difference = ",time_difference
    print*,"deduce_whether_to_issue_values timestep = ",timestep
    print*,"deduce_whether_to_issue_values frequency = ",frequency
    !modulo_result = modulo(timestep,frequency)
    !print*,"modulo_result = ",modulo_result
    !if (modulo_result .eq. 0) then
    !  select_value = .true.
    !else
    !  select_value = .false.
    !end if
    !****** MODIF *****
    print*,"deduce_whether_to_issue_values22222"
    print*,"deduce_whether_to_issue_values select_value = ",select_value
