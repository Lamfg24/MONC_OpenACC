...
contains
  ...
  subroutine register_command_receive()
    ...
    !print*,"register_command_receive command_buffer = ",command_buffer
    call unlock_mpi()
  ...
  integer function data_receive(mpi_datatype, num_elements, source, dump_data, data_dump_id, description_data)
    ...
    if (present(dump_data)) then
      ...
      call lock_mpi()
      print*,"data_receive0000"
      call mpi_irecv(dump_data, num_elements, mpi_datatype, source, tag_to_use, MPI_COMM_WORLD, request, ierr) ! les données de modélisation des coeurs monc à partir de la fonction send_data_to_io_server du fichier iobridge.F90 ! LAMBERT
      print*,"data_receive tag_to_use = ",tag_to_use
      call unlock_mpi()
