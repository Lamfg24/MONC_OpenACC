...
!   real(wp) :: k1, k2, k3
   ...
   if (l_prf_cfrac) then
    ...
    if (cloud_mass *cf_liquid> ql_small .and. &
      ...
      ! be changed 
      if (l_2mr) then
        dnumber2=dmass/(rain_params%c_x*(mu_qc/3.0)*(50.0E-6)**3)
        if (isnan(dnumber2) .eqv. .true.) print*,"NANNNNNN"
      end if
...
!convert back to grid mean
      !print*,"dmass = ",dmass
      !print*,"dnumber1 = ",dnumber1
      !print*,"dnumber2 = ",dnumber2
      ...
      cloud_mass=cloud_mass*cf_liquid !for aerosol processing below
          ...
          if (l_separate_rain) then
             if (l_aaut .and. l_process) then
                ...
                aerosol_procs(i_am4, i_aaut%id)%column_data(k)=-damass ! ActiveSolLiquid
                aerosol_procs(i_am5, i_aaut%id)%column_data(k)=damass ! ActiveSolRain
             end if
          end if
       end if
    enddo
