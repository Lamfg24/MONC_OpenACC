...
contains
  ...
  subroutine query_distributions(params, qfields, cffields, icall, D0_array)
    ...
    real(wp), intent(inout), optional :: D0_array(:)
...
!    end if
    ...
    do k=1, nz
      ...
      !!end if
      !KF.>
      if ((m1(k) .gt. 0.0_wp) .and. (m2(k) .ge. 1.0_wp)) then
        D0 = (m1(k)/(params%c_x * m2(k)))**(1/params%d_x) !(m1(k)/params%c_x)**(1/params%d_x)
        !D0 = ((6*m1(k))/(pi*params%density*m2(k)))**(1.0/3.0)
        D0_array(k) = D0
        !if (params%id .eq. 3 .and. ntimestep .eq. 2400) print*,"m1(k) = ", m1(k)," m2(k) = ", m2(k)," D0 = ",D0
      end if
    end do
...
!       end if
 ...
 subroutine query_distributions_test(ntimestep, params, qfields, cffields, icall, D0_array)
...
!    end if
    ...
    do k=1, nz
      ...
      if ((m1(k) .gt. 0.0_wp) .and. (m2(k) .ge. 1.0_wp)) then
        D0 = ((6*m1(k))/(pi*params%density*m2(k)))**(1.0/3.0)
        D0_array(k) = D0
        if (params%id .eq. 3 .and. ntimestep .eq. 3600) print*,"k = ",k," m1(k) = ", m1(k)," m2(k) = ", m2(k)," D0 = ",D0
      end if
    end do
...
!       end if
 ...
 end subroutine query_distributions_test
