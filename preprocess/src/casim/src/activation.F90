...
contains
  ...
  subroutine activate(dt, ntimestep, cloud_mass, cloud_number, w, rho, dnumber, dmac, T, p,  &
...
! This is cloud_mass_preap2 which is the OLD cloud mass.
    !print*,"cloud_mass = ", cloud_mass
    !print*,"cloud_number = ", cloud_number
    ...
    case(4)
      ...
      if (w > w_small .and. (sum(aerophys%N(:))+sum(dustphys%N(:))) > ccn_tidy) then
        ...
        if (.not. l_warm) then
           call AbdulRazzakGhan2000_dust(ntimestep, w, p, T, aerophys, aerochem, dnccn_all, Smax, aeroact, &
