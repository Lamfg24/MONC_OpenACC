...
contains
  ...
  function get_array_double(field_starts, field_ends, data_dump, key)
    ...
    if (elements .ge. ARRAY_STEP_THRESHOLD) then 
       ...
       do while (current_start_index .lt. end_index)
          ...
          ! transfer function = Interprets the bitwise representation of SOURCE in memory as if it is the representation of a variable or array of the same type and type parameters as MOLD.
  ...
  function get_array_double_from_monc(io_configuration, source, data_id, data_dump, key)
    ...
    monc_location=c_get_integer(io_configuration%monc_to_index, conv_to_string(source))
    !print*,"monc_location = ",monc_location
    !print*,"key = ",key
    get_array_double_from_monc=get_array_double(&
         io_configuration%registered_moncs(monc_location)%field_start_locations(data_id), &
         io_configuration%registered_moncs(monc_location)%field_end_locations(data_id), data_dump, key)  ! LAMBERT recupère les tableaux avec des nombres pas des bytes
    !print*,"get_array_double_from_monc = ",get_array_double_from_monc
    !print*,"size(get_array_double_from_monc) = ",size(get_array_double_from_monc)
    !print*,"minval(get_array_double_from_monc) = ",minval(get_array_double_from_monc)
    !print*,"maxval(get_array_double_from_monc) = ",maxval(get_array_double_from_monc)
