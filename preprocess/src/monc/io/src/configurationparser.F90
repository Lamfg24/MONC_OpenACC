...
contains
  ...
  recursive function get_io_xml(filename, funit_num) result(io_xml)
    ...
    do while (ierr == 0)
      ...
      !print*,"adjusted_io_line = ",adjusted_io_line
  ...
  subroutine configuration_parse(options_database_string, raw_configuration)
    ...
    character, dimension(:), intent(in) :: raw_configuration ! = io_xml_configuration
    ...
    do iter = 1,350
      if (options_database_string(iter,1) .eq. "l_thoff") then
        read(options_database_string(iter,2),*) logicnum
        l_thoff = logicnum
      else if (options_database_string(iter,1) .eq. "time_basis") then
        read(options_database_string(iter,2),*) logicnum
        time_basis = logicnum
      end if
    end do
  ...
  function get_data_value_from_hashmap_by_field_name(collection, field_name)
    ...
    !print*,"get_data_value_from_hashmap_by_field_name00"
    ...
    !print*,"get_data_value_from_hashmap_by_field_name01"
  ...
  function get_data_value_from_map_by_field_name(collection, field_name)
    ...
    !print*,"get_data_value_from_map_by_field_name00"
    generic=>c_get_generic(collection, field_name)
    !print*,"get_data_value_from_map_by_field_name field_name = ",field_name
    !print*,"associated(generic) = ",associated(generic)
    if (associated(generic)) then
      select type(generic)
      type is (data_values_type)
        !print*,"get_data_value_from_map_by_field_nameaa"
        get_data_value_from_map_by_field_name=>generic
      end select
    else
      !print*,"get_data_value_from_map_by_field_namebb"
      get_data_value_from_map_by_field_name=>null()
    end if
    !print*,"get_data_value_from_map_by_field_name01"
  ...
  logical function get_diagnostic_field_configuration(io_configuration, field_name, field_namespace, diagnostic_config)
    ...
    do i=1, size(io_configuration%diagnostics)
      !print*,"io_configuration%diagnostics(i)%collective = ", &
      !           io_configuration%diagnostics(i)%collective
  ...
  logical function get_prognostic_field_configuration(io_configuration, field_name, field_namespace, &
    ...
    do i=1, io_configuration%number_of_data_definitions
      do j=1, io_configuration%data_definitions(i)%number_of_data_fields
        !print*,"io_configuration%data_definitions(i)%fields(j)%collective = ", &
        !         io_configuration%data_definitions(i)%fields(j)%collective
