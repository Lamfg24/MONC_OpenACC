...
module timestepper_mod
  ...
  !procedure (component_func), pointer :: timestep_callback_petsc_solver_ptr => no_component
  ...
  procedure (component_func), pointer :: timestep_callback_flux_budget_ptr => no_component
  procedure (component_func), pointer :: timestep_callback_diagnostics_averaged_ptr => no_component
  procedure (component_func), pointer :: initialisation_callback_randomnoise_ptr => no_component
...
contains
  ...
  subroutine init_timestepper(current_state)
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "rad_interval") then
      ...
      else if (current_state%options_database_string(iter,1) .eq. "constant_w_profile_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        current_state%constant_w_profile_enabled = logicnum
        ...
        read(current_state%options_database_string(iter,2),*) logicnum
        current_state%pdf_analysis_enabled = logicnum
      ...
      !else if (current_state%options_database_string(iter,1) .eq. "petsc_solver_enabled") then
      !  read(current_state%options_database_string(iter,2),*) logicnum
      !  if (logicnum .eqv. .true.) timestep_callback_petsc_solver_ptr => timestep_callback_petsc_solver
      ...
      else if (current_state%options_database_string(iter,1) .eq. "flux_budget_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        current_state%flux_budget_enabled = logicnum
        if (logicnum .eqv. .true.) timestep_callback_flux_budget_ptr => timestep_callback_flux_budget
      ...
      else if (current_state%options_database_string(iter,1) .eq. "diagnostics_averaged_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        if (logicnum .eqv. .true.) timestep_callback_diagnostics_averaged_ptr => timestep_callback_diagnostics_averaged
        current_state%diagnostics_averaged_enabled = logicnum
      end if
    end do
  ...
  subroutine timestep(current_state)
    ...
    integer :: i, iter
    real(kind=DEFAULT_PRECISION) :: mod_freq, sum_qv, sum_zqv, dzniter
    real(kind=DEFAULT_PRECISION), dimension(:) :: rho_user(161)
    ...
    if (current_state%diagnostic_file_averaged_sample_frequency .eq. 0) then
      current_state%modulo_number_averaged_sample = -1.0
    else
      if (current_state%time_frequency_enabled .eqv. .true.) then
        mod_freq = modulo(current_state%time, float(current_state%diagnostic_file_averaged_sample_frequency))
        if ((mod_freq - current_state%dtm) .le. 0.0) then
          current_state%modulo_number_averaged_sample = int(mod_freq)
        else
          current_state%modulo_number_averaged_sample = -1.0
        end if
      else
        current_state%modulo_number_averaged_sample = modulo(current_state%timestep, &
                                                      current_state%diagnostic_file_averaged_sample_frequency)
      end if
    end if
    ...
    if (current_state%tracking_variables_enabled .eqv. .true.) then
      ...
      print*,"       "
      !do iter=1,161
      !  dzniter = current_state%global_grid%configuration%vertical%dzn(iter)
      !  sum_qv = sum_qv + sum(current_state%qv%data(iter,:,:))*60*60*dzniter*rho_user(iter)
      !  sum_zqv = sum_zqv + sum(current_state%zqv%data(iter,:,:))*60*60*dzniter*rho_user(iter)
      !end do
      !print*,"sum_qv5 = ", sum_qv
      !print*,"sum_zqv5 = ", sum_zqv
      !sum_qv = 0.0
      !sum_zqv = 0.0
      print*,"############################################################################"
      print*,"       "
    ...
    if (current_state%tracking_variables_enabled .eqv. .true.) then
      ...
      !  sum_zqv = sum_zqv + sum(current_state%zqv%data(iter,:,:))*60*60*dzniter*rho_user(iter)
      !end do
      !print*,"sum_qv6 = ", sum_qv
      !print*,"sum_zqv6 = ", sum_zqv
      !sum_qv = 0.0
      !sum_zqv = 0.0
      print*,"############################################################################"
      print*,"############################################################################"
  ...
  subroutine timestep_column_dynamics_group_contents(current_state)!, group_descriptor)
    ...
    do while (current_state%column_global_x .le. &
      ...
      do while (current_state%column_global_y .le. &
        ...
        call timestep_callback_flux_budget_ptr(current_state)
  ...
  subroutine timestep_whole_solver_group_contents(current_state)!, group_descriptor)
    ...
    !call timestep_callback_petsc_solver_ptr(current_state)
  ...
  subroutine timestep_column_pressure_terms_group_contents(current_state)!, group_descriptor)
    ...
    do while (current_state%column_global_x .le. &
      ...
      do while (current_state%column_global_y .le. &
        ...
        !if (current_state%time_to_apply_random_noise .ge. (current_state%time) .and. &
        !        current_state%time_to_apply_random_noise .lt. (current_state%time + current_state%dtm)) then
        if ((current_state%time .ge. current_state%time_to_apply_random_noise) .and. &
          (current_state%time .le.(current_state%time_to_apply_random_noise + current_state%duration_random_noise))) then
          print*,"essai"
          call initialisation_callback_randomnoise_ptr(current_state)
        end if
  ...
  subroutine timestep_whole_last_group_contents(current_state)!, group_descriptor)
    ...
    call timestep_callback_diagnostics_averaged_ptr(current_state)
