...
module optionsdatabase_mod
  ...
  ! Maximum of arguments fixed at 50 so other uses of options_database_real should begin from 51
  ! Maximum of arguments fixed at 50 so other uses of options_database_string should begin from 51
  subroutine load_command_line_into_options_database(state, options_database_real, options_database_string, &
                                                     report_option)
    !type(hashmap_type), intent(inout) :: options_database
    ...
    do i=1,arguments
      ...
      if (index(specific_arg, "--") .eq. 1) then
        if (equals_posn .gt. 0) then
          ...
          options_database_string(i,1) = specific_arg(3:equals_posn-1)
          options_database_string(i,2) = specific_arg(equals_posn+1 : len(specific_arg))
        else
          ! If no equals then it is a logical switch (to true)
          type_of_config = LOGICAL_TYPE
          options_database_string(i,1) = specific_arg(3:equals_posn-1)
        end if
        ! remember LOGICAL_TYPE=0, INTEGER_TYPE=1, REAL_TYPE=2, STRING_TYPE=3
        options_database_real(i,1) = type_of_config
