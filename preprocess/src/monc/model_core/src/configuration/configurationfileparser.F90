...
contains
  ...
  !!        and process
  subroutine parse_configuration_file(state, options_database_real, options_database_string, user_configuration_file)
    ...
    call process_configuration_file(state, options_database_real, options_database_string, user_configuration_file, .true., &
         USER_FILE_ID)
  ...
  recursive subroutine process_configuration_file(state, options_database_real, options_database_string, filename, is_user_file,&
       file_id)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    ...
    integer :: file_status, comment_point, index_1
    integer, dimension(2) :: shape_arr
    logical :: continue_parsing, found_global
    character(len=10000) :: raw_line

    index_1 = state%config_args + 1
    shape_arr = shape(options_database_string)
    found_global=.false.
    ...
    do while (continue_parsing)
      ...
      if (is_iostat_end(file_status)) then
        ...
        if (len_trim(raw_line) .gt. 0) then
          call process_configuration_line(options_database_real, options_database_string, raw_line, is_user_file, &
               found_global, index_1)
          index_1 = index_1 + 1
          if (index_1 .gt. shape_arr(1)) then
            call log_master_log(LOG_ERROR, "Configuration arrays size not sufficient, see state.F90")
          end if
        end if
      end if
      state%config_args = state%config_args +1
    end do
    close(file_id)
  ...
  recursive subroutine process_configuration_line(options_database_real, options_database_string, raw_line, is_user_file, &
       found_global, index_1)
    !type(hashmap_type), intent(inout) :: options_database
    real(kind=DEFAULT_PRECISION), dimension(1200,500), intent(inout) :: options_database_real
    character(len=STRING_LENGTH), dimension(1200,2), intent(inout) :: options_database_string
    ...
    integer :: mode, start_split, end_split, index_1
    character(len=500) :: config_key
    character(len=10000) :: config_value
    ...
    if (mode .ge. 1 .and. raw_line(1:1) .ne. '#' .and. raw_line(1:1) .ne. '!') then
      ...
      if (has_multiple_values(config_value)) then
        call process_configuration_array(options_database_real, options_database_string, config_key, config_value, &
                                          mode, index_1)
      else
        if (is_key_array_index_specifier(config_key) .or. mode .eq. 2) then
          ...
          options_database_string(index_1,1) = config_key
          options_database_string(index_1,2) = config_value
          !print*,"options_database_string(index_1,2) = ",options_database_string(index_1,2)
      !      call store_configuration(options_database, config_key, config_value)
          !if (is_user_file .and. .not. found_global) then
          !  print*,"index_1 = ",index_1
          !  found_global=parse_global_configuration_if_available(options_database_real, options_database_string)
          !end if
        end if
      end if
   end if
  ...
  logical function parse_global_configuration_if_available(options_database_real, options_database_string)
    !type(hashmap_type), intent(inout) :: options_database
    real(kind=DEFAULT_PRECISION), dimension(1000,500), intent(inout) :: options_database_real
    character(len=STRING_LENGTH), dimension(1000,2), intent(inout) :: options_database_string

    integer :: iter

!     do iter = 51,600
!       if (options_database_string(iter,1) .eq. "global_configuration") then
...
!     end do
!
!     print*,"parse_global_configuration_if_available_check"
  ...
  subroutine process_configuration_array(options_database_real, options_database_string, config_key, config_value, &
                                         mode, index_1)
    !type(hashmap_type), intent(inout) :: options_database
    real(kind=DEFAULT_PRECISION), dimension(1200,500), intent(inout) :: options_database_real
    character(len=STRING_LENGTH), dimension(1200,2), intent(inout) :: options_database_string
    character(500), intent(in) :: config_key
    character(10000), intent(in) :: config_value
    integer, intent(in) :: mode, index_1
    ...
    !else if (mode==2) then
    !  index_2=options_get_array_size(options_database, config_key)+1
    end if
    !
    raw_value=config_value
    !print*,"parsed_config_key = ",parsed_config_key
    !print*,"config_value = ",config_value
    comma_posn=scan(raw_value, ",")
    !print*,"comma_posn = ",comma_posn
    do while (comma_posn .gt. 0)
      options_database_string(index_1,1) = parsed_config_key
      read(raw_value(1:comma_posn-1),*) realnum
      options_database_real(index_1,index_2) = realnum

      !call store_configuration(options_database_real, options_database_string, parsed_config_key, &
      !                         trim(adjustl(raw_value(1:comma_posn-1))), index)
      raw_value=raw_value(comma_posn+1:)
      comma_posn=scan(raw_value, ",")
      index_2 = index_2 + 1
    end do
    read(raw_value,*) realnum
    options_database_real(index_1,index_2) = realnum
  ...
  subroutine store_configuration(options_database_real, options_database_string, config_key, config_value, array_index)
    !type(hashmap_type), intent(inout) :: options_database
    real(kind=DEFAULT_PRECISION), dimension(1000,500), intent(inout) :: options_database_real
    character(len=STRING_LENGTH), dimension(1000,2), intent(inout) :: options_database_string
    character(*), intent(in) :: config_key, config_value
