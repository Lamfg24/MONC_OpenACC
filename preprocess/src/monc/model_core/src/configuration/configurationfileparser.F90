...
contains
  ...
  !!        and process
  subroutine parse_configuration_file(state, user_configuration_file)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    character(*), intent(in) :: user_configuration_file

    call process_configuration_file(state, user_configuration_file, .true., &
         USER_FILE_ID)
  ...
  recursive subroutine process_configuration_file(state, filename, is_user_file,&
       file_id)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    ...
    shape_arr = shape(state%options_database_string)
    found_global=.false.
    ...
    do while (continue_parsing)
      ...
      if (is_iostat_end(file_status)) then
        ...
        if (len_trim(raw_line) .gt. 0) then
          call process_configuration_line(state, raw_line, is_user_file, &
  ...
  recursive subroutine process_configuration_line(state, raw_line, is_user_file, &
       found_global, index_1)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    ...
    if (mode .ge. 1 .and. raw_line(1:1) .ne. '#' .and. raw_line(1:1) .ne. '!') then
      ...
      if (has_multiple_values(config_value)) then
        call process_configuration_array(state, config_key, config_value, &
                                          mode, index_1)
      else
        if (is_key_array_index_specifier(config_key) .or. mode .eq. 2) then
          ...
          state%options_database_string(index_1,1) = config_key
          state%options_database_string(index_1,2) = config_value
...
!     print*,"parse_global_configuration_if_available_check"
  ...
  subroutine process_configuration_array(state, config_key, config_value, &
                                         mode, index_1)
    !type(hashmap_type), intent(inout) :: options_database
    type(model_state_type), intent(inout) :: state
    ...
    do while (comma_posn .gt. 0)
      state%options_database_string(index_1,1) = parsed_config_key
      read(raw_value(1:comma_posn-1),*) realnum
      state%options_database_real(index_1,index_2) = realnum
