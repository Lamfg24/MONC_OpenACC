...
module setfluxlook_mod
  ...
  public initialisation_callback_setfluxlook, timestep_callback_setfluxlook

contains

  subroutine initialisation_callback_setfluxlook(current_state)
    ...
    if (current_state%use_surface_boundary_conditions)then
      if (current_state%type_of_surface_boundary_conditions .eq. PRESCRIBED_SURFACE_FLUXES) then
        ...
        if(.not. current_state%passive_q .and. current_state%number_q_fields > 0)then
            current_state%fbuoy=current_state%fbuoy+current_state%cq(1)*current_state%surface_vapour_flux*G
        end if
        ...
        ! surface vapour (surface_vapour_mixing_ratio) set to saturated value (see read_config)
          if (current_state%saturated_surface)then
            ...
            do iter = 1,current_state%config_args
              if (current_state%options_database_string(iter,1) .eq. "surface_vapour_mixing_ratio") then
                read(current_state%options_database_string(iter,2),*) realnum
                current_state%surface_vapour_mixing_ratio = realnum
              end if
            end do
          endif
        ...
        ! Set theta_v
          ...
          if (.not. current_state%passive_q .and. current_state%number_q_fields > 0) then
            current_state%theta_virtual_surf = current_state%theta_surf +  &
                  ...
                  current_state%cq(1)*current_state%surface_vapour_mixing_ratio
          end if
  ...
  end subroutine initialisation_callback_setfluxlook

  subroutine timestep_callback_setfluxlook(current_state)
    type(model_state_type), intent(inout), target :: current_state
  ...
  end subroutine timestep_callback_setfluxlook
  ...
  subroutine set_look(current_state)
    ...
    current_state%mo_l = -(sum(current_state%lookup_table_ustr)/size(current_state%lookup_table_ustr))**3/&
                          (von_karman_constant*current_state%fbuoy) ! mean Monin Obukhov length
  ...
  subroutine set_flux(current_state)
    ...
    if (current_state%type_of_surface_boundary_conditions == PRESCRIBED_SURFACE_FLUXES) then  ! Prescribed surface fluxes
      ...
      if (.not. current_state%passive_q) then                                                      
        current_state%fbuoynew=current_state%fbuoynew+current_state%cq(1)*current_state%surface_vapour_flux*G
      end if
  ...
  subroutine read_configuration(current_state)
    ...
    integer :: ncid, time_dim, iter, intnum, iter1, iter2, size_array
    integer :: number_input_humidities, number_times, number_check
    logical :: logicnum, surface_forcing_file_enabled
    real(kind=DEFAULT_PRECISION) :: realnum

    number_times = 0

    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "use_surface_boundary_conditions") then
        read(current_state%options_database_string(iter,2),*) logicnum
        current_state%use_surface_boundary_conditions = logicnum
      end if
    end do

    if (current_state%use_surface_boundary_conditions)then
      do iter = 1,current_state%config_args
        if (current_state%options_database_string(iter,1) .eq. "type_of_surface_boundary_conditions") then
          read(current_state%options_database_string(iter,2),*) intnum
          current_state%type_of_surface_boundary_conditions = intnum
        else if (current_state%options_database_string(iter,1) .eq. "use_time_varying_surface_values") then
          read(current_state%options_database_string(iter,2),*) logicnum
          current_state%use_time_varying_surface_values = logicnum
        else if (current_state%options_database_string(iter,1) .eq. "surface_forcing_file_enabled") then
          read(current_state%options_database_string(iter,2),*) logicnum
          surface_forcing_file_enabled = logicnum
        else if (current_state%options_database_string(iter,1) .eq. "surface_conditions_file") then
          input_file = current_state%options_database_string(iter,2)
        else if (current_state%options_database_string(iter,1) .eq. "max_change_buoyancy_flux") then
          read(current_state%options_database_string(iter,2),*) realnum
          max_change_buoyancy_flux = realnum
        end if
      end do
    ...
    if (surface_forcing_file_enabled .eqv. .false.) then
      if (current_state%use_time_varying_surface_values)then
        allocate(surface_boundary_input_times(MAX_SURFACE_INPUTS))
        surface_boundary_input_times=0.0

        do iter = 1,current_state%config_args
          if (current_state%options_database_string(iter,1) .eq. "surface_boundary_input_times") then
            surface_boundary_input_times = current_state%options_database_real(iter,:)
          end if
        end do
        number_times = size(surface_boundary_input_times)
      ...
      if (current_state%type_of_surface_boundary_conditions == PRESCRIBED_SURFACE_FLUXES)then
        ...
        do iter1 = 1,current_state%config_args
          size_array = 0
          if (current_state%options_database_string(iter1,1) .eq. "surface_sensible_heat_flux") then
            if (isnan(current_state%options_database_real(iter1,1))) then
              allocate(surface_sensible_heat_flux(1))
              read(current_state%options_database_string(iter1,2),*) realnum
              surface_sensible_heat_flux = realnum
            else
              ...
              allocate(surface_sensible_heat_flux(size_array))
              surface_sensible_heat_flux = current_state%options_database_real(iter1,1:size_array)
            end if
          else if (current_state%options_database_string(iter1,1) .eq. "surface_latent_heat_flux") then
            if (isnan(current_state%options_database_real(iter1,1))) then
              allocate(surface_latent_heat_flux(1))
              read(current_state%options_database_string(iter1,2),*) realnum
              surface_latent_heat_flux = realnum
            else
              do iter2 = 1,size(current_state%options_database_real(iter1,:))
                if (isnan(current_state%options_database_real(iter1,iter2))) then
                  exit
              ...
              allocate(surface_latent_heat_flux(size_array))
              surface_latent_heat_flux = current_state%options_database_real(iter1,1:size_array)
            end if
          end if
        end do
        ...
        call check_time_arrays(current_state, surface_sensible_heat_flux, number_times, "surface_sensible_heat_flux")
        call check_time_arrays(current_state, surface_latent_heat_flux, number_times, "surface_latent_heat_flux")
        ...
        do iter1 = 1,current_state%config_args
          size_array = 0
          if (current_state%options_database_string(iter1,1) .eq. "surface_temperatures") then
            if (isnan(current_state%options_database_real(iter1,1))) then
              allocate(surface_temperatures(1))
              read(current_state%options_database_string(iter1,2),*) realnum
              surface_temperatures = realnum
            else
              do iter2 = 1,size(current_state%options_database_real(iter1,:))
                if (isnan(current_state%options_database_real(iter1,iter2))) then
                  exit
              ...
              allocate(surface_temperatures(size_array))
              surface_temperatures = current_state%options_database_real(iter1,1:size_array)
            end if
          else if (current_state%options_database_string(iter,1) .eq. "units_surface_temp") then
            units_surface_temp = current_state%options_database_string(iter,2)
          else if (current_state%options_database_string(iter1,1) .eq. "surface_humidities") then
            if (isnan(current_state%options_database_real(iter1,1))) then
              allocate(surface_humidities(1))
              read(current_state%options_database_string(iter1,2),*) realnum
              surface_humidities = realnum
            else
              do iter2 = 1,size(current_state%options_database_real(iter1,:))
                if (isnan(current_state%options_database_real(iter1,iter2))) then
                  exit
              ...
              allocate(surface_humidities(size_array))
              surface_humidities = current_state%options_database_real(iter1,1:size_array)
            end if
          end if
        end do
        ...
        call check_time_arrays(current_state, surface_temperatures, number_times, "surface_temperatures")
        call check_time_arrays(current_state, surface_humidities, number_times, "surface_humidities")
      end if
...
!  BELOW COPIED FROM KIDREADER - WOULD BE GOOD TO MAKE THESE UTILITY FUNCTIONS...
  ...
  subroutine check_time_arrays(current_state, array, number_times, key)
    real(kind=DEFAULT_PRECISION), dimension(MAX_SURFACE_INPUTS), intent(inout) :: array
    type(model_state_type), intent(inout), target :: current_state
    ...
    if (size(array) .ne. number_times) then
       call log_master_log(LOG_WARN, "Model configured to use_time_varying_surface_values, "//         &
