...
contains
  ...
  subroutine swap_and_smooth_classic(current_state, old_smoother)
    ...
    real(kind=DEFAULT_PRECISION) :: c1, c2, existing_value, diff_w, diff_zw
    ...
    do k=1,current_state%global_grid%size(Z_INDEX)
      ...
      if (current_state%constant_w_profile_enabled .eqv. .true. .and. &
                  current_state%time .le. current_state%constant_w_profile_timer) then
        diff_w = current_state%w%data(k,y_index,x_index) - current_state%global_grid%configuration%vertical%w_init(k)
        current_state%w%data(k,y_index,x_index) = current_state%w%data(k,y_index,x_index) - diff_w
        diff_zw = current_state%zw%data(k,y_index,x_index) - current_state%global_grid%configuration%vertical%w_init(k)
        current_state%zw%data(k,y_index,x_index) = current_state%zw%data(k,y_index,x_index) - diff_zw
      end if
      if (current_state%th%active) then
