...
module smagorinsky_mod
  ...
  public calculate_half_squared_strain_rate, &
         calculate_richardson_number, calculate_thermal_dissipation_rate, initialisation_callback_smagorinsky, &
         timestep_callback_smagorinsky, finalisation_callback_smagorinsky
...
contains
  ...
  subroutine initialisation_callback_smagorinsky(current_state)
    ...
    do iter = 1,current_state%config_args
      ...
      else if (current_state%options_database_string(iter,1) .eq. "smag-subb") then
        read(current_state%options_database_string(iter,2),*) realnum
        subb = realnum
      else if (current_state%options_database_string(iter,1) .eq. "smag-subc") then
        read(current_state%options_database_string(iter,2),*) realnum
        subc = realnum
      end if
    end do
    ...
    if (.not. current_state%passive_q) then 
       iqv = 1!get_q_index(standard_q_names%VAPOUR, 'smagorinsky')
       current_state%water_vapour_mixing_ratio_index=iqv
       
       iql = 2!get_q_index(standard_q_names%CLOUD_LIQUID_MASS, 'smagorinsky')
       current_state%liquid_water_mixing_ratio_index=iql
    endif
       
  end subroutine initialisation_callback_smagorinsky
  
  !> For each none halo cell this will calculate the subgrid terms for viscosity and diffusion
  !! @param current_state The current model state
  subroutine timestep_callback_smagorinsky(current_state)
    type(model_state_type), target, intent(inout) :: current_state
    ...
    if (.not. current_state%halo_column) then
      if (.not. current_state%use_viscosity_and_diffusion) then
        ...
        if (current_state%field_stepping == FORWARD_STEPPING) then
          ...
          richardson_number=calculate_richardson_number(current_state, ssq, current_state%zth, current_state%zqv,&
          current_state%zql)
        end if
  ...
  end subroutine timestep_callback_smagorinsky

  !> Called when the model is finishing up, will finalise the halo communications represented by the state
  !! @param current_state The current model state
  subroutine finalisation_callback_smagorinsky(current_state)
    type(model_state_type), target, intent(inout) :: current_state
  ...
  end subroutine finalisation_callback_smagorinsky
  ...
  function calculate_richardson_number(current_state, ssq, th, qv, ql)
    type(model_state_type), target, intent(inout) :: current_state
    real(kind=DEFAULT_PRECISION), dimension(:), intent(in) :: ssq
    type(prognostic_field_type), intent(inout) :: th, qv, ql
    !type(prognostic_field_type), dimension(:), intent(inout) :: qv
    ...
    if (.not. current_state%passive_th) then
      if (.not. current_state%passive_q) then
        if (RICHARDSON_NUMBER_CALCULATION .eq. 2) then
          calculate_richardson_number=moist_ri_2(current_state, ssq, th, qv, ql)
        else
          calculate_richardson_number=moist_ri_1(current_state, ssq, th, qv, ql)
        end if
      else
        do k=2, current_state%local_grid%size(Z_INDEX)-1
  ...
  function moist_ri_2(current_state, ssq, th, qv, ql)
    type(model_state_type), target, intent(inout) :: current_state
    real(kind=DEFAULT_PRECISION), dimension(:), intent(in) :: ssq
    type(prognostic_field_type), intent(inout) :: th, qv, ql
    ...
    do K=2,current_state%local_grid%size(Z_INDEX)-1
      ...
      qt_k = qv%data(k, current_state%column_local_y, current_state%column_local_x)+&
           ql%data(k, current_state%column_local_y, current_state%column_local_x)
      qt_kp1 = qv%data(k+1, current_state%column_local_y, current_state%column_local_x)+&
           ql%data(k+1, current_state%column_local_y, current_state%column_local_x)
      thlpr_k = th%data(k, current_state%column_local_y, current_state%column_local_x) -&
           rlvap_over_cp*ql%data(k, current_state%column_local_y, current_state%column_local_x)*&
      ...
      thlpr_kp1 = th%data(k+1, current_state%column_local_y, current_state%column_local_x) - &
           rlvap_over_cp*ql%data(k+1, current_state%column_local_y, current_state%column_local_x)*&
           current_state%global_grid%configuration%vertical%prefrcp(k)
  ...
  !! @param ssq The half squared strain rate
  !! @returns The Richardson number calculated here for a specific column
  function moist_ri_1(current_state, ssq, th, qv, ql)
    type(model_state_type), target, intent(inout) :: current_state
    real(kind=DEFAULT_PRECISION), dimension(:), intent(in) :: ssq
    type(prognostic_field_type), intent(inout) :: th, qv, ql
    !type(prognostic_field_type), dimension(:), intent(inout) :: q
    ...
    do k=2, current_state%local_grid%size(Z_INDEX)
      liquid_water_static_energy_temperature = &
           ...
           ql%data(k, current_state%column_local_y, current_state%column_local_x)
      if (k==2) then
        total_water_substance = qv%data(&
             k, current_state%column_local_y, current_state%column_local_x) + &
             ql%data(k, current_state%column_local_y, current_state%column_local_x)
      else
        total_water_substance=total_water_substance_p1
      end if
      total_water_substance_p1 = qv%data(k+1, current_state%column_local_y, current_state%column_local_x) + &
           ql%data(k+1, current_state%column_local_y, current_state%column_local_x)
      if (k .le. current_state%local_grid%size(Z_INDEX)-1) then
        ...
        qloading = current_state%cq(current_state%water_vapour_mixing_ratio_index)*&
             ...
             (ql%data(k+1, current_state%column_local_y, current_state%column_local_x)-liquid_water_at_kp1)
