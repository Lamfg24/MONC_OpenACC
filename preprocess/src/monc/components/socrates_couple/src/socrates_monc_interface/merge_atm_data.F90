...
contains
  ...
  subroutine merge_data(current_state, socrates_opt, socrates_derived_fields, merge_fields, mcc)
    ...
    integer ::  iter
    logical :: logicnum, rcemip_ozone_enabled


    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "l_rcemip_ozone") then
        read(current_state%options_database_string(iter,2),*) logicnum
        rcemip_ozone_enabled = logicnum
      end if
    end do
...
! cloud_number at 1+mcc%cut is not yet set.
    do k = 2, k_top
       ...
       ! vapour mixing ratio
       merge_fields%qv_n(k+mcc%cut) = current_state%qv%data(k_top+2-k, jcol, icol)
       if ( socrates_opt%cloud_representation == 2) then
          if (socrates_opt%mphys_nq_l > 0) then
             merge_fields%ql_n(k+mcc%cut) =  &
                  current_state%ql%data(k_top+2-k, jcol, icol)
          ...
          if (socrates_opt%mphys_nq_r > 0) then
             merge_fields%ql_n(k+mcc%cut) = merge_fields%ql_n(k+mcc%cut) + &
                  ...
                  (current_state%qr%data(k_top+2-k, jcol, icol))
          endif
          if (socrates_opt%mphys_nq_i > 0) then
             merge_fields%qi_n(k+mcc%cut) = &
                  current_state%qi%data(k_top+2-k, jcol, icol)
          endif
          ...
          if (socrates_opt%mphys_nq_s > 0) then
             merge_fields%qi_n(k+mcc%cut) = merge_fields%qi_n(k+mcc%cut) +         &
                  ...
                  (current_state%qs%data(k_top+2-k, jcol, icol))
          endif
          if (socrates_opt%mphys_nq_g > 0) then
             merge_fields%qi_n(k+mcc%cut) = merge_fields%qi_n(k+mcc%cut) +         &
                  merge_fields%graupfac*                &
                  (current_state%qg%data(k_top+2-k, jcol, icol))
          endif
    ...
    merge_fields%qv_n(mcc%cut+1)= &
         (current_state%qv%data(k_top, jcol, icol) + mcc%q_n(mcc%cut))/2.0
    ...
    if (rcemip_ozone_enabled .eqv. .true.) &
