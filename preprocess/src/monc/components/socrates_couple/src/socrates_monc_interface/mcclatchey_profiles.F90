...
contains
  ...
  subroutine read_mcclatchey_profiles(current_state, mcc)
    ...
    character(MAX_FILE_LEN) :: mcc_temp_input_file, mcc_vapour_input_file, mcc_ozone_input_file
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "mcc_temperature_profile") then
          mcc_temp_input_file = ADJUSTL(current_state%options_database_string(iter,2))
      else if (current_state%options_database_string(iter,1) .eq. "mcc_vapour_profile") then
          mcc_vapour_input_file = ADJUSTL(current_state%options_database_string(iter,2))
      else if (current_state%options_database_string(iter,1) .eq. "mcc_ozone_profile") then
          mcc_ozone_input_file = ADJUSTL(current_state%options_database_string(iter,2))
      end if
    end do
    ...
    if (log_get_logging_level() .ge. LOG_DEBUG) then
       call log_master_log(LOG_DEBUG, &
            "Reading in McClatchey pressure and temperature from:"//trim(mcc_temp_input_file) )
    endif
    if (mcc_temp_input_file .eq. "None") then
      stop
    else
      call check_mcc_status(nf90_open(path = trim(mcc_temp_input_file), mode = nf90_nowrite, ncid = ncid))
      ...
      call read_mcc_variables(trim(mcc_temp_input_file), ncid, plev_dim, &
    ...
    !mcc_vapour_input_file =  &
    !     options_get_string(current_state%options_database, "mcc_vapour_profile")
    if (log_get_logging_level() .ge. LOG_DEBUG) then
       call log_master_log(LOG_DEBUG, &
            "Reading in McClatchey vapour profile from:"//trim(mcc_vapour_input_file) )
    endif
    if (mcc_vapour_input_file .eq. "None") then
      stop
    else
      call check_mcc_status(nf90_open(path = trim(mcc_vapour_input_file), mode = nf90_nowrite, ncid = ncid))
      call read_mcc_variables(trim(mcc_vapour_input_file), ncid, plev_dim, VAPOUR_PROFILE_KEY, temp_mcc_3d)
    ...
    !mcc_ozone_input_file =  &
    !     options_get_string(current_state%options_database, "mcc_ozone_profile")
    if (log_get_logging_level() .ge. LOG_DEBUG) then
       call log_master_log(LOG_DEBUG, &
            "Reading in McClatchey ozone profile from:"//trim(mcc_ozone_input_file) )
    endif
    if (mcc_ozone_input_file .eq. "None") then
      stop
    else
      call check_mcc_status(nf90_open(path = trim(mcc_ozone_input_file), mode = nf90_nowrite, ncid = ncid))
      call read_mcc_variables(trim(mcc_ozone_input_file), ncid, plev_dim, OZONE_PROFILE_KEY, temp_mcc_3d)
