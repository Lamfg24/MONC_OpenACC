...
logical :: logicnum, rcemip_gases_enabled
...
! Set default values of control variables.
  ...
  do iter = 1,current_state%config_args
    if (current_state%options_database_string(iter,1) .eq. "spectral_file_lw") then
        lw_control%spectral_file = ADJUSTL(current_state%options_database_string(iter,2))
    else if (current_state%options_database_string(iter,1) .eq. "i_gas_overlap_lw") then
        read(current_state%options_database_string(iter,2),*) intnum
        lw_control%i_gas_overlap   = intnum
    else if (current_state%options_database_string(iter,1) .eq. "i_cloud_representation") then
        read(current_state%options_database_string(iter,2),*) intnum
        lw_control%i_cloud_representation   = intnum
    else if (current_state%options_database_string(iter,1) .eq. "i_water_lw") then
        read(current_state%options_database_string(iter,2),*) intnum
        lw_control%i_st_water   = intnum
    else if (current_state%options_database_string(iter,1) .eq. "i_ice_lw") then
        read(current_state%options_database_string(iter,2),*) intnum
        lw_control%i_st_ice   = intnum
    else if (current_state%options_database_string(iter,1) .eq. "i_scatter_method_lw") then
        read(current_state%options_database_string(iter,2),*) intnum
        lw_control%i_scatter_method   = intnum
