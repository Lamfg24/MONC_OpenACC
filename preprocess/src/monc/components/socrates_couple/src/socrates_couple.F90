...
module socrates_couple_mod
  ...
  public initialisation_callback_socrates, finalisation_callback_socrates, timestep_callback_socrates
contains
...
!     socrates_couple_get_descriptor%published_fields_on_off = .true.
...
!     socrates_couple_get_descriptor%published_fields(11)="surface_up_shortwave"cloud_reff_tot
...
!   end function socrates_couple_get_descriptor
  ...
  subroutine initialisation_callback_socrates(current_state)
    ...
    allocate(current_state%cloud_reff_tot(k_top), current_state%longwave_hr_tot(k_top), &
             current_state%shortwave_hr_tot(k_top))
    ...
    allocate( current_state%tend_3d_tabs_lw(k_top,y_nohalos,x_nohalos))
    allocate( current_state%tend_3d_tabs_sw(k_top,y_nohalos,x_nohalos))
    allocate( current_state%tend_3d_tabs_total(k_top,y_nohalos,x_nohalos))
    allocate( current_state%tend_pr_tot_th_lw(k_top))
    allocate( current_state%tend_pr_tot_th_sw(k_top))
    allocate( current_state%tend_pr_tot_th_total(k_top))
    allocate( current_state%tend_pr_tot_tabs_lw(k_top))
    allocate( current_state%tend_pr_tot_tabs_sw(k_top))
    allocate( current_state%tend_pr_tot_tabs_total(k_top))
    ...
    call read_spectrum( sw_control%spectral_file, sw_spectrum )
  ...
  end subroutine initialisation_callback_socrates

  !> Called for each column per timestep this will apply a forcing term
  !> to the aerosol fields
  !! @param current_state The current model state
  subroutine timestep_callback_socrates(current_state)
    type(model_state_type), target, intent(inout) :: current_state
...
!!$              albedoin2 = alb2_var
    ...
    current_state%cloud_reff%data(:,target_y_index,target_x_index) = &
               socrates_derived_fields%cloud_reff(:,target_y_index,target_x_index)
    current_state%lwrad_hr%data(:,target_y_index,target_x_index) =  &
               socrates_derived_fields%lwrad_hr(:,target_y_index,target_x_index)
    current_state%swrad_hr%data(:,target_y_index,target_x_index) = &
               socrates_derived_fields%swrad_hr(:,target_y_index,target_x_index)

    current_state%cloud_reff_tot(:) = current_state%cloud_reff_tot(:) + &
         (socrates_derived_fields%cloud_reff(:,target_y_index, target_x_index))
    current_state%longwave_hr_tot(:) =  current_state%longwave_hr_tot(:) + &
         (socrates_derived_fields%lwrad_hr(:,target_y_index, target_x_index))
    current_state%shortwave_hr_tot(:) = current_state%shortwave_hr_tot(:) + &
         (socrates_derived_fields%swrad_hr(:,target_y_index, target_x_index))
  ...
  end subroutine timestep_callback_socrates

  subroutine finalisation_callback_socrates(current_state)
    type(model_state_type), target, intent(inout) :: current_state

!    deallocate(merge_fields%t, merge_fields%qv, &
!         ql_socrates, qi_socrates)
    if (allocated(current_state%tend_3d_tabs_lw)) deallocate(current_state%tend_3d_tabs_lw)
    if (allocated(current_state%tend_3d_tabs_sw)) deallocate(current_state%tend_3d_tabs_sw)
    if (allocated(current_state%tend_3d_tabs_total)) deallocate(current_state%tend_3d_tabs_total)

    if (allocated(current_state%tend_pr_tot_th_lw)) deallocate(current_state%tend_pr_tot_th_lw)
    if (allocated(current_state%tend_pr_tot_th_sw)) deallocate(current_state%tend_pr_tot_th_sw)
    if (allocated(current_state%tend_pr_tot_th_total)) deallocate(current_state%tend_pr_tot_th_total)
    if (allocated(current_state%tend_pr_tot_tabs_lw)) deallocate(current_state%tend_pr_tot_tabs_lw)
    if (allocated(current_state%tend_pr_tot_tabs_sw)) deallocate(current_state%tend_pr_tot_tabs_sw)
    if (allocated(current_state%tend_pr_tot_tabs_total)) deallocate(current_state%tend_pr_tot_tabs_total)

  end subroutine finalisation_callback_socrates
  ...
  subroutine compute_component_tendencies(current_state, cxn, cyn, txn, tyn)
    ...
    current_state%tend_3d_tabs_lw(:,tyn,txn)    = current_state%sth_lw%data(:,cyn,cxn) &
                                    * current_state%global_grid%configuration%vertical%rprefrcp(:)
    current_state%tend_3d_tabs_sw(:,tyn,txn)    = current_state%sth_sw%data(:,cyn,cxn) &
                                    * current_state%global_grid%configuration%vertical%rprefrcp(:)
    current_state%tend_3d_tabs_total(:,tyn,txn) = current_state%tend_3d_tabs_lw(:,tyn,txn) + &
                                                  current_state%tend_3d_tabs_sw(:,tyn,txn)

   ! Add local tendency fields to the profile total
    current_state%tend_pr_tot_th_lw(:) = current_state%tend_pr_tot_th_lw(:) + current_state%sth_lw%data(:,cyn,cxn)
    current_state%tend_pr_tot_th_sw(:) = current_state%tend_pr_tot_th_sw(:) + current_state%sth_sw%data(:,cyn,cxn)
    current_state%tend_pr_tot_th_total(:) = current_state%tend_pr_tot_th_total(:) &
                              + current_state%sth_lw%data(:,cyn,cxn) + current_state%sth_sw%data(:,cyn,cxn)

    current_state%tend_pr_tot_tabs_lw(:) = current_state%tend_pr_tot_tabs_lw(:) + current_state%tend_3d_tabs_lw(:,tyn,txn)
    current_state%tend_pr_tot_tabs_sw(:) = current_state%tend_pr_tot_tabs_sw(:) + current_state%tend_3d_tabs_sw(:,tyn,txn)
    current_state%tend_pr_tot_tabs_total(:) = current_state%tend_pr_tot_tabs_total(:) + current_state%tend_3d_tabs_total(:,tyn,txn)
