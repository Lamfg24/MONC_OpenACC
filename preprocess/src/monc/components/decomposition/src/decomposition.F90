...
contains
  ...
  subroutine init_callback_decomposition(current_state)
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "decomposition_enabled") then
        ...
        if (logicnum .eqv. .false.) return
      else if (current_state%options_database_string(iter,1) .eq. "decomposition_method") then
        method = current_state%options_database_string(iter,2)
      else if (current_state%options_database_string(iter,1) .eq. "halo_depth") then
  ...
  end subroutine init_callback_decomposition
  ...
  subroutine two_dim_decomposition(current_state)
    ...
    integer :: ierr, local_sum_grid_x, local_sum_grid_y, total_nbr_element, nbr_element_global_grid, nbr_element_local_grid
    ...
    call apply_dimension_bounds(current_state, Y_INDEX, merge(current_state%global_grid%size(Y_INDEX), 1, & ! merge : give the number of size(Y_INDEX) if active
    ...
    nbr_element_local_grid = current_state%local_grid%size(X_INDEX) * current_state%local_grid%size(Y_INDEX)
    total_nbr_element = current_state%global_grid%size(X_INDEX) * current_state%global_grid%size(Y_INDEX)
    call mpi_reduce(nbr_element_local_grid , nbr_element_global_grid, 1, &
                         MPI_INT, MPI_SUM, 0, current_state%parallel%monc_communicator, ierr)
    if (current_state%timestep .eq. 1) then
      if (current_state%parallel%my_rank == 0) then
        if (nbr_element_global_grid .ne. total_nbr_element) then
          call log_log(LOG_DEBUG, "Problem of MONC parallelization: total number of parallized local grid elements"//&
                                  "is not equal to global total number of elements")
          call exit
        end if
      end if
    end if
  ...
  subroutine apply_dimension_bounds(current_state, dim, dim_size, dim_processes, dim_my_rank)
    ...
    integer :: dimension_division, dimension_extra, rank
  ...
  subroutine apply_halo_information_and_allocate_neighbours(current_state)
    ...
    !integer :: halo_depth, intnum, iter
