...
module conditional_diagnostics_column_mod
  ...
  !character(len=STRING_LENGTH), allocatable :: cond_request ! , cond_long
...
!          timestep_callback_diagnostics_column, finalisation_callback_conditional_diagnostics_column
  ...
  public ncond, gpts_total, initialisation_callback_conditional_diagnostics_column, &
         timestep_callback_diagnostics_column, finalisation_callback_conditional_diagnostics_column
...
contains
  ...
  subroutine initialisation_callback_conditional_diagnostics_column(current_state)
    ...
    if (ncond .lt. 1) call log_master_log(LOG_ERROR, &
...
!                        (w_zn .lt. wdwncrit) .and. (qppt .gt. vpptcrit)  )
      ...
      if (l_cond) then
        current_state%w_zn_cln(k) = current_state%w_zn_cln(k) + w_zn ! [w_zn] = m.s-1 => vertical velocity
        current_state%w_zn2_cln(k) = current_state%w_zn2_cln(k) + w_zn2 ! [w_zn2] = m2.s-2 => vertical velocity variance
        current_state%tmp_th_cln(k) = current_state%tmp_th_cln(k) + tmp_th ! [tmp_th] = K => potential temperature
        current_state%wth_cln(k) = current_state%wth_cln(k) + wth ! [wth] = K.m.s-1 => flux of potential temperature
        current_state%th_pr_cln(k) = current_state%th_pr_cln(k) + th_pr ! [th_pr] = K => potential temperature anomalies
        current_state%wth_pr_cln(k) = current_state%wth_pr_cln(k) + wth_pr ! [wth_pr] = K.m.s-1 => flux of potential temperature anomalies
        current_state%thv_pr_cln(k) = current_state%thv_pr_cln(k) + thv_pr ! [thv_pr] = K => virtual potential temperature anomalies
        current_state%wthv_pr_cln(k) = current_state%wthv_pr_cln(k) + wthv_pr ! [wthv_pr] = K.m.s-1 => flux of virtual potential temperature
        current_state%th_pr2_cln(k) = current_state%th_pr2_cln(k) + th_pr2 ! [th_pr2] = K2 => variance of potential temperature
        current_state%wthsg_cln(k) = current_state%wthsg_cln(k) + wthsg ! [wthsg] = ??? => flux diffusion coefficient by temperature
        current_state%w_zn3_cln(k) = current_state%w_zn3_cln(k) + w_zn3 ! [w_zn3] = m3.s-3 =>
        current_state%relhum_cln(k) = current_state%relhum_cln(k) + relhum ! => relative humidity
        current_state%tmp_u_cln(k) = current_state%tmp_u_cln(k) + tmp_u ! [tmp_u] = m.s-1 => zonal velocity
        current_state%tmp_v_cln(k) = current_state%tmp_v_cln(k) + tmp_v ! [tmp_v] = m.s-1 => meridional velovity
        current_state%wu_cln(k) = current_state%wu_cln(k) + wu ! [wu] = m2.s-2 => flux components wu
        current_state%wv_cln(k) = current_state%wv_cln(k) + wv ! [wv] = m2.s-2 => flux components wv
        current_state%wusg_cln(k) = current_state%wusg_cln(k) + wusg ! [wusg] = ??? => flux viscosity coefficient follonwing u
        current_state%wvsg_cln(k) = current_state%wvsg_cln(k) + wvsg ! [wvsg] = ??? => flux viscosity coefficient follonwing v
        current_state%TdegK_cln(k) = current_state%TdegK_cln(k) + TdegK ! [TdegK] = C => temperature
        current_state%th_h_cln(k) = current_state%th_h_cln(k) + th_h ! [th_h] = K => air temperature variation
        current_state%th_h_pr1_cln(k) = current_state%th_h_pr1_cln(k) + th_h_pr1 ! [th_h_pr1] = K => potential temperature anomalies variation
        current_state%th_h_pr2_cln(k) = current_state%th_h_pr2_cln(k) + th_h_pr2 ! [th_h_pr2] = K2 => variance ofpotential temperature anomalies variation
        current_state%qvli_cln(k) = current_state%qvli_cln(k) + qvli ! [qvli] = kg.kg-1 =>
        current_state%qvli_pr_cln(k) = current_state%qvli_pr_cln(k) + qvli_pr ! [qvli_pr] = kg.kg-1 => anomalies
        current_state%qvli_pr2_cln(k) = current_state%qvli_pr2_cln(k) + qvli_pr2 ! [qvli_pr2] = kg2.kg-2 => variance anomalies

        current_state%qppt_cln(k) = current_state%qppt_cln(k) + qppt ! [qppt] = kg.kg-1 =>
        current_state%qppt_pr_cln(k) = current_state%qppt_pr_cln(k) + qppt_pr ! [qppt_pr] = kg.kg-1 => anomalies
        current_state%qppt_pr2_cln(k) = current_state%qppt_pr2_cln(k) + qppt_pr2 ! [qppt_pr2] = kg2.kg-2 => variance anomalies
        current_state%wqvli_pr_cln(k) = current_state%wqvli_pr_cln(k) + wqvli_pr ! [wqvli_pr] = kg.kg-1.m.s-1 => flux anomalies by w component
        current_state%wqppt_pr_cln(k) = current_state%wqppt_pr_cln(k) + wqppt_pr ! [wqppt_pr] = kg.kg-1.m.s-1 => flux anomalies by w component
      end if
