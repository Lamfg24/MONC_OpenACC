...
module scalar_diagnostics_mod
  ...
  logical :: species_water_path_diag_user
  real(kind=DEFAULT_PRECISION) :: vwp_layer_min, vwp_layer_max, lwp_layer_min, lwp_layer_max, rwp_layer_min, rwp_layer_max, &
                                  iwp_layer_min, iwp_layer_max, swp_layer_min, swp_layer_max, gwp_layer_min, gwp_layer_max, &
                                  tot_iwp_layer_min, tot_iwp_layer_max
...
contains
  ...
  subroutine initialisation_callback_scalar_diagnostics(current_state)
    ...
    real(kind=DEFAULT_PRECISION) :: realnum, logicnum
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "qlcrit") then
        ...
        qlcrit = realnum
      !else if (current_state%options_database_string(iter,1) .eq. "species_water_path_diag_user") then
      !  read(current_state%options_database_string(iter,2),*) logicnum
      !  species_water_path_diag_user = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "vwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        vwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "vwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        vwp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "lwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        lwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "lwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        lwp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "rwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        rwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "rwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        rwp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "iwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        iwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "iwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        iwp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "swp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        swp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "swp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        swp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "gwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        gwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "gwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        gwp_layer_max = realnum
      else if (current_state%options_database_string(iter,1) .eq. "tot_iwp_layer_min") then
        read(current_state%options_database_string(iter,2),*) realnum
        tot_iwp_layer_min = realnum
      else if (current_state%options_database_string(iter,1) .eq. "tot_iwp_layer_max") then
        read(current_state%options_database_string(iter,2),*) realnum
        tot_iwp_layer_max = realnum
      end if
    end do
    ...
    if (.not. current_state%passive_q .and. current_state%number_q_fields .gt. 0) then
       ...
       allocate(current_state%vwp_usr(y_size_local, x_size_local), current_state%lwp_usr(y_size_local, x_size_local))
       ...
       if ((current_state%casim_enabled .eqv. .false.) .and. (current_state%simplecloud_enabled .eqv. .false.)) then
        ...
        if (current_state%rain_water_mixing_ratio_index > 0) then
            ...
            allocate(current_state%rwp_usr(y_size_local, x_size_local))
        endif
        if (current_state%ice_water_mixing_ratio_index > 0) then
            ...
            allocate(current_state%iwp_usr(y_size_local, x_size_local))
            allocate(current_state%tot_iwp_usr(y_size_local, x_size_local))
        endif
        if (current_state%snow_water_mixing_ratio_index > 0) then
            ...
            allocate(current_state%swp_usr(y_size_local, x_size_local))
        endif
        if (current_state%graupel_water_mixing_ratio_index > 0) then
            ...
            allocate(current_state%gwp_usr(y_size_local, x_size_local))
        endif
      end if
    endif
  ...
  subroutine timestep_callback_scalar_diagnostics(current_state)
    ...
    real(kind=DEFAULT_PRECISION) :: altitude_k
        ...
        if (.not. current_state%passive_q) then! .and. current_state%number_q_fields .gt. 0) then
          ...
          if ((current_state%casim_enabled .eqv. .true.) .or. (current_state%simplecloud_enabled .eqv. .true.)) then
            ...
            if (current_state%ice_water_mixing_ratio_index > 0) then
              ...
              current_state%iwp_usr(:,:)=0.0
              ! total ice water path (iwp + swp + gwp) for each column
              current_state%tot_iwp(:,:)=0.0
              current_state%tot_iwp_usr(:,:)=0.0
            endif
            ! snow water path for each column
            if (current_state%snow_water_mixing_ratio_index > 0) then
              current_state%swp(:,:)=0.0
              current_state%swp_usr(:,:)=0.0
            end if
            ! graupel water path for each column
            if (current_state%graupel_water_mixing_ratio_index > 0) then
              current_state%gwp(:,:)=0.0
              current_state%gwp_usr(:,:)=0.0
            end if
          end if
       end if
    ...
    if (.not. current_state%halo_column) then
      ...
      if (.not. current_state%passive_q .and. current_state%number_q_fields .gt. 0) then
          ...
          !     current_state%number_q_fields .ge. current_state%water_vapour_mixing_ratio_index) then
            do k = 2, current_state%local_grid%size(Z_INDEX)
                altitude_k = current_state%global_grid%configuration%vertical%z(k)
                ...
                if (altitude_k .ge. vwp_layer_min .and. altitude_k .le. vwp_layer_max) then
                  current_state%vwp_usr(target_y_index, target_x_index)=current_state%vwp_usr(target_y_index, target_x_index) &
                    +dz_rhon_fac(k)*current_state%qv%data(k, current_y_index, current_x_index)
                end if
                ...
                if (altitude_k .ge. lwp_layer_min .and. altitude_k .le. lwp_layer_max) then
                  current_state%lwp_usr(target_y_index, target_x_index)=current_state%lwp_usr(target_y_index, target_x_index) &
                    +dz_rhon_fac(k)*current_state%ql%data(k, current_y_index, current_x_index)
                end if
            enddo
            if ((current_state%casim_enabled .eqv. .true.) .or. (current_state%simplecloud_enabled .eqv. .true.)) then
            !if (current_state%rain_water_mixing_ratio_index > 0) then
                do k = 2, current_state%local_grid%size(Z_INDEX)
                  ...
                  if (altitude_k .ge. rwp_layer_min .and. altitude_k .le. rwp_layer_max) then
                    current_state%rwp_usr(target_y_index, target_x_index)=current_state%rwp_usr(target_y_index, target_x_index) &
                      +dz_rhon_fac(k)*current_state%qr%data(k,current_y_index, current_x_index)
                  end if
                enddo
            !endif
            !if (current_state%ice_water_mixing_ratio_index > 0) then
                do k = 2, current_state%local_grid%size(Z_INDEX)
                  ...
                  if (altitude_k .ge. iwp_layer_min .and. altitude_k .le. iwp_layer_max) then
                    current_state%iwp_usr(target_y_index, target_x_index)=current_state%iwp_usr(target_y_index, target_x_index) &
                      +dz_rhon_fac(k)*current_state%qi%data(k,current_y_index, current_x_index)
                  end if
                enddo
                ...
                current_state%tot_iwp_usr(target_y_index, target_x_index) = &
                    current_state%tot_iwp_usr(target_y_index, target_x_index)+ &
                    current_state%iwp_usr(target_y_index, target_x_index)
            !endif
            !if (current_state%snow_water_mixing_ratio_index > 0) then
                do k = 2, current_state%local_grid%size(Z_INDEX)
                  altitude_k = current_state%global_grid%configuration%vertical%z(k)
                  ...
                  if (altitude_k .ge. swp_layer_min .and. altitude_k .le. swp_layer_max) then
                    current_state%swp_usr(target_y_index, target_x_index)=current_state%swp_usr(target_y_index, target_x_index) &
                      +dz_rhon_fac(k)*current_state%qs%data(k,current_y_index, current_x_index)
                  end if
                enddo
                current_state%tot_iwp(target_y_index, target_x_index)=current_state%tot_iwp(target_y_index, target_x_index)+ &
                ...
                current_state%tot_iwp_usr(target_y_index, target_x_index) = &
                    current_state%tot_iwp_usr(target_y_index, target_x_index)+ &
                    current_state%swp_usr(target_y_index, target_x_index)
            !endif
            !if (current_state%graupel_water_mixing_ratio_index > 0) then
                do k = 2, current_state%local_grid%size(Z_INDEX)
                  altitude_k = current_state%global_grid%configuration%vertical%z(k)
                  ...
                  if (altitude_k .ge. gwp_layer_min .and. altitude_k .le. gwp_layer_max) then
                    current_state%gwp_usr(target_y_index, target_x_index)=current_state%gwp_usr(target_y_index, target_x_index) &
                      +dz_rhon_fac(k)*current_state%qg%data(k,current_y_index, current_x_index)
                  end if
                enddo
                current_state%tot_iwp(target_y_index, target_x_index)=current_state%tot_iwp(target_y_index, target_x_index)+ &
                ...
                current_state%tot_iwp_usr(target_y_index, target_x_index) = &
                    current_state%tot_iwp_usr(target_y_index, target_x_index)+ &
                    current_state%gwp_usr(target_y_index, target_x_index)
            !endif
          !end if
          end if
      end if
