...
module halo_communication_mod
  ...
  integer :: halo_depth_com, timestep_halo
...
contains
  ...
  subroutine blocking_halo_swap(current_state, halo_swap_state, copy_to_halo_buffer, &
    ...
    timestep_halo = current_state%timestep
  ...
  subroutine copy_buffer_to_corner(local_grid, halo_buffer, field_data, corner_loc, &
    ...
    field_data(local_grid%local_domain_start_index(Z_INDEX):&
         ...
         y_target_index, x_target_index)=halo_buffer(:,9,halo_page)
    field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-1, y_target_index+1, corner_loc .lt. 3), x_target_index)=&
         halo_buffer(:,8,halo_page)
    field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         y_target_index, merge(x_target_index-1, x_target_index+1, corner_loc == 1 .or.&
         corner_loc == 3))= halo_buffer(:,7,halo_page)
    field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-1, y_target_index+1, corner_loc .lt. 3), &
         merge(x_target_index-1, x_target_index+1, corner_loc == 1 .or.&
         corner_loc == 3))= halo_buffer(:,6,halo_page)
    if (halo_depth_com .eq. 3) then
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-2, y_target_index+2,corner_loc .lt. 3), &
         x_target_index) = halo_buffer(:,5,halo_page)
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         y_target_index, merge(x_target_index-2,x_target_index+2, corner_loc == 1 .or. corner_loc == 3)) = &
         halo_buffer(:,4,halo_page)
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-2, y_target_index+2, corner_loc .lt. 3), &
         merge(x_target_index-1,x_target_index+1, corner_loc == 1 .or. corner_loc == 3)) = halo_buffer(:,3,halo_page)
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-1, y_target_index+1,corner_loc .lt. 3), &
         merge(x_target_index-2,x_target_index+2, corner_loc == 1 .or. corner_loc == 3)) = halo_buffer(:,2,halo_page)
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
         local_grid%local_domain_end_index(Z_INDEX),&
         merge(y_target_index-2, y_target_index+2,corner_loc .lt. 3), &
         merge(x_target_index-2,x_target_index+2, corner_loc == 1 .or. corner_loc == 3)) = halo_buffer(:,1,halo_page)
    end if
  ...
  subroutine copy_buffer_to_field(local_grid, halo_buffer, field_data, dim, target_index, &
    ...
    ! around and order of
    ! messages being sent. Thalo_pagehis is not an issue if the neighbours are different
    if (dim == X_INDEX) then
      field_data(local_grid%local_domain_start_index(Z_INDEX):&
           local_grid%local_domain_end_index(Z_INDEX),&
  ...
  subroutine copy_corner_to_buffer(local_grid, halo_buffer, field_data, corner_loc, &
    ...
    halo_buffer(:,4,halo_page) = field_data(:, merge(y_source_index-1, y_source_index+1,   &
         ...
         corner_loc == 3))
    if (halo_depth_com .eq. 3) then
      halo_buffer(:,5,halo_page) = field_data(:,merge(y_source_index-2, y_source_index+2, corner_loc .lt. 3),x_source_index)
      halo_buffer(:,6,halo_page) = field_data(:,y_source_index,merge(x_source_index-2,x_source_index+2, &
         corner_loc == 1 .or. corner_loc == 3))
      halo_buffer(:,7,halo_page) = field_data(:,merge(y_source_index-2, y_source_index+2,corner_loc .lt. 3), &
         merge(x_source_index-1,x_source_index+1, corner_loc == 1 .or. corner_loc == 3))
      halo_buffer(:,8,halo_page) = field_data(:,merge(y_source_index-1, y_source_index+1,corner_loc .lt. 3), &
         merge(x_source_index-2,x_source_index+2, corner_loc == 1 .or. corner_loc == 3))
      halo_buffer(:,9,halo_page) = field_data(:,merge(y_source_index-2, y_source_index+2,corner_loc .lt. 3), &
         merge(x_source_index-2,x_source_index+2, corner_loc == 1 .or. corner_loc == 3))
    end if
      ...
      if (halo_swap_neighbours(i)%halo_corners .gt. 0) then
        ...
        if (halo_depth_com .le. 2 ) then
          ...
          allocate(halo_swap_neighbours(i)%recv_corner_buffer(local_grid%size(Z_INDEX), 4, &
              halo_swap_neighbours(i)%halo_corners))
        else if (halo_depth_com .eq. 3) then
          allocate(halo_swap_neighbours(i)%send_corner_buffer(local_grid%size(Z_INDEX), 9, &
              halo_swap_neighbours(i)%halo_corners))
          allocate(halo_swap_neighbours(i)%recv_corner_buffer(local_grid%size(Z_INDEX), 9, &
              halo_swap_neighbours(i)%halo_corners))
        end if
      end if
    end do    
...
! start_index --- start_index +1 --- .... --- end_index -1 --- end_index for halo_depth = 2
!   source2
              else if (j==2) then
! start_index --- start_index +1 --- .... --- end_index -1 --- end_index for halo_depth = 1 or 2
!   source1          source2
                  source_index = current_state%local_grid%local_domain_start_index(i) + &
...
!                                                               source2
              ...
              if (j==1) then
                ...
                source_index = current_state%local_grid%local_domain_start_index(i)+1
              else if (j==3) then
                source_index = current_state%local_grid%local_domain_start_index(i)+2
              else if (j==4) then
                source_index = current_state%local_grid%local_domain_end_index(i)-2
              else if (j==5) then
                source_index = current_state%local_grid%local_domain_end_index(i)-1
              else if (j==6) then
                source_index = current_state%local_grid%local_domain_end_index(i)
              end if
             end if
       ...
       do j = 1, size(current_state%local_grid%corner_neighbours, 1)
          if (current_state%parallel%my_rank .ne. &
            ...
            if (halo_depth .le. 2) then
              ...
              x_source_index = merge(current_state%local_grid%local_domain_start_index(X_INDEX)+2,&
                    current_state%local_grid%local_domain_end_index(X_INDEX)-2, j==1 .or. j==3)
              y_source_index  =merge(current_state%local_grid%local_domain_start_index(Y_INDEX)+2,&
                    current_state%local_grid%local_domain_end_index(Y_INDEX)-2, j==1 .or. j==2)
            end if
            pid_location = get_pid_neighbour_location(halo_swap_state%halo_swap_neighbours, &
  ...
  subroutine copy_buffer_data_for_prognostics(current_state, halo_swap_state, copy_halo_buffer_to_field, &
    ...
    if (halo_swap_state%halo_depth .le. 2) then
      hstart=merge(2, 1, halo_swap_state%halo_depth==1)
      hend=merge(3, 4, halo_swap_state%halo_depth==1)
    else if (halo_swap_state%halo_depth .eq. 3) then
      hstart = 1
      hend = 6
    end if
        ...
        if (current_state%parallel%my_rank .ne. current_state%local_grid%neighbours(i,j)) then
          if (halo_swap_state%halo_depth .le. 2) then
            ...
            if (j==halo_swap_state%cell_match(i, 1)) then
              ...
              target_index=current_state%local_grid%local_domain_end_index(i)+1
            else if (j==halo_swap_state%cell_match(i, 5)) then
              target_index=current_state%local_grid%local_domain_end_index(i)+2
            else if (j==halo_swap_state%cell_match(i, 6)) then
              target_index=current_state%local_grid%local_domain_end_index(i)+3
            end if
          end if
          pid_location=get_pid_neighbour_location(halo_swap_state%halo_swap_neighbours, &
               current_state%local_grid%neighbours(i,j),&
      ...
      do j=size(current_state%local_grid%corner_neighbours, 1),1,-1
        if (current_state%parallel%my_rank .ne. &
          ...
          if (halo_depth_com .le. 2) then
            ...
            x_target_index=merge(3, current_state%local_grid%local_domain_end_index(X_INDEX)+1,&
                j==1 .or. j==3)
            y_target_index=merge(3, current_state%local_grid%local_domain_end_index(Y_INDEX)+1, &
                j==1 .or. j==2)
          end if
          pid_location=get_pid_neighbour_location(halo_swap_state%halo_swap_neighbours, &
               current_state%local_grid%corner_neighbours(j,1), &
    ...
    do i=1,size(local_grid%corner_neighbours, 1)
      if (my_rank .eq. local_grid%corner_neighbours(i,1)) then
        if (halo_depth_com .le. 2) then
          ...
          if (i==1) then
            y_source_index=local_grid%local_domain_end_index(Y_INDEX)-2
            x_source_index=local_grid%local_domain_end_index(X_INDEX)-2
            y_target_index=1
            x_target_index=1
          else if (i==2) then
            y_source_index=local_grid%local_domain_end_index(Y_INDEX)-2
          ...
          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              ...
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+1, x_source_index)
          ...
          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              ...
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index, x_source_index+1)
          ...
          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              ...
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+1, x_source_index+1)

          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_target_index+2, x_target_index)=&
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+2, x_source_index)

          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_target_index, x_target_index+2)=&
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index, x_source_index+2)

          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_target_index+2, x_target_index+1)=&
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+2, x_source_index+1)

          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_target_index+1, x_target_index+2)=&
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+1, x_source_index+2)

          field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_target_index+2, x_target_index+2)=&
              field_data(local_grid%local_domain_start_index(Z_INDEX):&
              local_grid%local_domain_end_index(Z_INDEX),&
              y_source_index+2, x_source_index+2)
        end if
      end if      
    end do    
  ...
  subroutine perform_local_data_copy_for_dimension(dim, my_rank, halo_depth, local_grid, &
    ...
    real(kind=DEFAULT_PRECISION), dimension(:,:,:), allocatable :: field_data_copy
    ...
    do i=hstart, hend
      if (local_grid%neighbours(dim,i) .eq. my_rank) then
        if (halo_depth .le. 2) then
          if (i==1) then
!   h1 --- h2 --- start_index --- start_index +1 --- .... --- end_index -1 --- end_index --- h3 --- h4 for halo_depth = 2
! target                                                         source
            target_index=1
            source_index=local_grid%local_domain_end_index(dim)-1
          else if (i==2) then
!   h1 --- h2 --- start_index --- start_index +1 --- .... --- end_index -1 --- end_index --- h3 --- h4 for halo_depth = 1 and 2
!        target                                                                 source
            target_index=2
...
!                                     source                                                      target
          ...
          if (i==1) then
!   h1 --- h2 --- h3 --- start_index --- start_index +1 --- start_index + 2 --- .... --- end_index -2 --- end_index -1 --- end_index --- h4 --- h5 --- h6 for halo_depth = 3
! target                                                                                    source
            target_index=1
            source_index=local_grid%local_domain_end_index(dim)-2
          else if (i==2) then
!   h1 --- h2 --- h3 --- start_index --- start_index +1 --- start_index + 2 --- .... --- end_index -2 --- end_index -1 --- end_index --- h4 --- h5 --- h6 for halo_depth = 3
!        target                                                                                             source
            target_index=2
...
!   h1 --- h2 --- h3 --- start_index --- start_index +1 --- start_index + 2 --- .... --- end_index -2 --- end_index -1 --- end_index --- h4 --- h5 --- h6 for halo_depth = 3
!                                            source                                                                                                  target
            target_index=local_grid%local_domain_end_index(dim)+3
            source_index=local_grid%local_domain_start_index(dim)+2
          end if
        end if
