...
module gridmanager_mod
  ...
  public initialise_callback_gridmanager, finalise_callback_gridmanager
...
contains
  ...
  subroutine initialise_callback_gridmanager(current_state)
    type(model_state_type), target, intent(inout) :: current_state

    integer :: dimensions, k, iter
    logical :: logicnum
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "passive_th") then
      ...
      else if (current_state%options_database_string(iter,1) .eq. "immersed_boundary_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        current_state%immersed%ib_enabled = logicnum
    ...
    if (current_state%immersed%ib_enabled) then
      call initialise_immersed_boundary(current_state) ! a revoir
    end if
    dimensions=1
  ...
  end subroutine initialise_callback_gridmanager

  !> Called as MONC exits, for good practice this will deallocate the memory used for the grids
  !! @param current_state The current model state_mod
  subroutine finalise_callback_gridmanager(current_state)
    type(model_state_type), target, intent(inout) :: current_state
    ...
    deallocate(vertical_grid%z, vertical_grid%zn, vertical_grid%dz, vertical_grid%dzn, vertical_grid%czb, vertical_grid%cza, &
         ...
         vertical_grid%u_init, vertical_grid%v_init, vertical_grid%w_init, vertical_grid%q_init,                              &
         vertical_grid%q_rand, vertical_grid%theta_rand, vertical_grid%w_subs, vertical_grid%w_rand, &
         vertical_grid%qv_force, vertical_grid%ql_force, vertical_grid%qr_force, vertical_grid%qi_force, &
         vertical_grid%qs_force, vertical_grid%qg_force, &
         vertical_grid%qAitkenSolMass_force, vertical_grid%qAccumSolMass_force, vertical_grid%qAccumInsolMass_force, &
         vertical_grid%qCoarseSolMass_force, vertical_grid%qCoarseDustMass_force, &
         vertical_grid%theta_force, vertical_grid%u_force, vertical_grid%v_force &
         )
  end subroutine finalise_callback_gridmanager

  !> Will initialise the immersed boundary ghost points, image points, surface
  !  exchange points etc.
  !! @param current_state The current model state_mod
  subroutine initialise_immersed_boundary(current_state) ! A revoir
...
!        llen = max(current_state%global_grid%configuration%vertical%zn(2), z0*50.0)
  ...
  subroutine initialise_verticalgrid_configuration_type(current_state)
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "nsmth") then
        read(current_state%options_database_string(iter,2),*) intnum
        nsmth = intnum
      end if
    end do
    call set_up_and_smooth_grid(current_state,current_state%global_grid%configuration%vertical, &
         current_state%global_grid%configuration%vertical%z_size, &
         current_state%global_grid%top(Z_INDEX), nsmth, &
         current_state%continuation_run)
  ...
  subroutine set_vertical_reference_profile(current_state, vertical_grid, kkp)
    ...
    !call set_qv_init_from_rh(current_state) !  a revoir
  ...
  subroutine calculate_initial_profiles(current_state, vertical_grid)
    ...
    integer :: iter, iter2, iter_z, size_array = 0
    ...
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qv     ! Vapour water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_ql     ! Liquid water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qr     ! Rain water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qi     ! Ice water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qg     ! Grauppel water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qs     ! Snow water_mixing_ratio
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qAitkenSolMass
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qAccumSolMass
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qAccumInsolMass
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qCoarseSolMass
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_qCoarseDustMass
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: z_init_pl_n      ! Initial node height values for n variables
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nl     ! Liquid water_number_concentration
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nr     ! Rain water_number_concentration
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_ni     ! Ice water_number_concentration
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_ng     ! Grauppel water_number_concentration
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_ns     ! Snow water_number_concentration
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nAitkenSolNumber
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nAccumSolNumber
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nAccumInsolNumber
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nCoarseSolNumber
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_nCoarseDustnumber
    ...
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: f_init_pl_w      ! Initial node values for w variable
    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: z_init_pl_w      ! Initial node height values for w variable
    ...
    logical :: l_init_pl_w     ! if .true. then initialize w field
    ...
    logical :: l_init_pl_n     ! if .true. then initialize n fields
    ...
    vertical_grid%w_init = 0.0_DEFAULT_PRECISION
    vertical_grid%theta_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qv_init = 0.0_DEFAULT_PRECISION
    vertical_grid%ql_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qr_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qi_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qs_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qAitkenSolMass_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qAccumSolMass_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qAccumInsolMass_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qCoarseSolMass_init = 0.0_DEFAULT_PRECISION
    vertical_grid%qCoarseDustMass_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nl_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nr_init = 0.0_DEFAULT_PRECISION
    vertical_grid%ni_init = 0.0_DEFAULT_PRECISION
    vertical_grid%ns_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nAitkenSolNumber_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nAccumSolNumber_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nAccumInsolNumber_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nCoarseSolNumber_init = 0.0_DEFAULT_PRECISION
    vertical_grid%nCoarseDustnumber_init = 0.0_DEFAULT_PRECISION
    ...
    do iter = 1,current_state%config_args
      ...
      if (current_state%options_database_string(iter,1) .eq. "casim_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        enable_casim = logicnum
      end if
      if (current_state%options_database_string(iter,1) .eq. "l_init_pl_theta") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_theta = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_theta") then
        ...
        allocate(z_init_pl_theta(size_array))
        z_init_pl_theta = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_theta") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_theta(size_array))
        f_init_pl_theta = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "z_thref") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_thref(size_array))
        z_thref = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_thref") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_thref(size_array))
        f_thref = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_u") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_u = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_u") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_init_pl_u(size_array))
        z_init_pl_u = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_u") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_u(size_array))
        f_init_pl_u = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_v") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_v = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_v") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_init_pl_v(size_array))
        z_init_pl_v = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_v") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_v(size_array))
        f_init_pl_v = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_w") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_w = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_w") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_init_pl_w(size_array))
        z_init_pl_w = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_w") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_w(size_array))
        f_init_pl_w = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_rh") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_rh = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_q") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_q = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_q") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_init_pl_q(size_array))
        z_init_pl_q = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qv") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qv(size_array))
        f_init_pl_qv = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_ql") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_ql(size_array))
        f_init_pl_ql = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qr") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qr(size_array))
        f_init_pl_qr = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qi") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qi(size_array))
        f_init_pl_qi = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qs") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qs(size_array))
        f_init_pl_qs = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qg") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qg(size_array))
        f_init_pl_qg = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qAitkenSolMass") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qAitkenSolMass(size_array))
        f_init_pl_qAitkenSolMass = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qAccumSolMass") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qAccumSolMass(size_array))
        f_init_pl_qAccumSolMass = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qAccumInsolMass") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qAccumInsolMass(size_array))
        f_init_pl_qAccumInsolMass = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qCoarseSolMass") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qCoarseSolMass(size_array))
        f_init_pl_qCoarseSolMass = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_qCoarseDustMass") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_qCoarseDustMass(size_array))
        f_init_pl_qCoarseDustMass = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_init_pl_n") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_init_pl_n = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "z_init_pl_n") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(z_init_pl_n(size_array))
        z_init_pl_n = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nl") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nl(size_array))
        f_init_pl_nl = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nr") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nr(size_array))
        f_init_pl_nr = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_ni") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_ni(size_array))
        f_init_pl_ni = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_ns") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_ns(size_array))
        f_init_pl_ns = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_ng") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_ng(size_array))
        f_init_pl_ng = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nAitkenSolNumber") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nAitkenSolNumber(size_array))
        f_init_pl_nAitkenSolNumber = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nAccumSolNumber") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nAccumSolNumber(size_array))
        f_init_pl_nAccumSolNumber = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nAccumInsolNumber") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nAccumInsolNumber(size_array))
        f_init_pl_nAccumInsolNumber = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nCoarseSolNumber") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nCoarseSolNumber(size_array))
        f_init_pl_nCoarseSolNumber = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "f_init_pl_nCoarseDustnumber") then
        do iter2 = 1,size(current_state%options_database_real(iter,:))
          if (isnan(current_state%options_database_real(iter,iter2))) then
            exit
          else
        ...
        allocate(f_init_pl_nCoarseDustnumber(size_array))
        f_init_pl_nCoarseDustnumber = current_state%options_database_real(iter,1:size_array)
      else if (current_state%options_database_string(iter,1) .eq. "l_thref") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_thref = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "l_matchthref") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_matchthref = logicnum
      end if
    end do
    ...
    if (.not. current_state%continuation_run) then  ! For continuations, ensure thref is that from checkpoint
      ...
      call check(nf90_inq_varid(ncdf_id, "thref", variable_id))
      call check(nf90_get_var(ncdf_id, variable_id, current_state%global_grid%configuration%vertical%thref( &
    ...
    if (l_init_pl_theta) then
      ...
      if (.not. current_state%continuation_run) then
        ...
        call check(nf90_inq_varid(ncdf_id, "th", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%th%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            ...
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            current_state%local_grid%start(X_INDEX)], &
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zth", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zth%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    if (l_init_pl_u) then
      ...
      if (.not. current_state%continuation_run) then
        ...
        call check(nf90_inq_varid(ncdf_id, "u", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%u%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zu", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zu%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    if (l_init_pl_v) then
      ...
      if (.not. current_state%continuation_run) then
        ...
        call check(nf90_inq_varid(ncdf_id, "v", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%v%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zv", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zv%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    if (l_init_pl_w) then
      call check_top(zztop, z_init_pl_w(size(z_init_pl_w)), 'z_init_pl_w')
      call check_input_levels(size(z_init_pl_w), size(f_init_pl_w), "f_init_pl_w")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_w(1:size(z_init_pl_w)), f_init_pl_w(1:size(f_init_pl_w)), &
         zgrid, current_state%global_grid%configuration%vertical%w_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%w%data(:,j,i) = current_state%global_grid%configuration%vertical%w_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "w", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%w%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zw", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zw%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    if (l_init_pl_q) then
      call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
      call check_input_levels(size(z_init_pl_q), size(f_init_pl_qv), "f_init_pl_qv")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qv(1:size(f_init_pl_qv)), &
         zgrid, current_state%global_grid%configuration%vertical%qv_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%qv%data(:,j,i) = current_state%global_grid%configuration%vertical%qv_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "qv", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qv%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqv", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqv%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
      call check_input_levels(size(z_init_pl_q), size(f_init_pl_ql), "f_init_pl_ql")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_ql(1:size(f_init_pl_ql)), &
         zgrid, current_state%global_grid%configuration%vertical%ql_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%ql%data(:,j,i) = current_state%global_grid%configuration%vertical%ql_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "ql", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%ql%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zql", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zql%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
      call check_input_levels(size(z_init_pl_q), size(f_init_pl_qr), "f_init_pl_qr")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qr(1:size(f_init_pl_qr)), &
         zgrid, current_state%global_grid%configuration%vertical%qr_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%qr%data(:,j,i) = current_state%global_grid%configuration%vertical%qr_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "qr", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qr%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqr", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqr%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qi), "f_init_pl_qi")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qi(1:size(f_init_pl_qi)), &
          zgrid, current_state%global_grid%configuration%vertical%qi_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qi%data(:,j,i) = current_state%global_grid%configuration%vertical%qi_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qi", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qi%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqi", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqi%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qs), "f_init_pl_qs")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qs(1:size(f_init_pl_qs)), &
          zgrid, current_state%global_grid%configuration%vertical%qs_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qs%data(:,j,i) = current_state%global_grid%configuration%vertical%qs_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qs", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qs%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqs", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqs%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qg), "f_init_pl_qg")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qg(1:size(f_init_pl_qg)), &
          zgrid, current_state%global_grid%configuration%vertical%qg_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qg%data(:,j,i) = current_state%global_grid%configuration%vertical%qg_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qg", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qg%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqg", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqg%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qAitkenSolMass), "f_init_pl_qAitkenSolMass")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qAitkenSolMass(1:size(f_init_pl_qAitkenSolMass)), &
          zgrid, current_state%global_grid%configuration%vertical%qAitkenSolMass_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qAitkenSolMass%data(:,j,i) = current_state%global_grid%configuration%vertical%qAitkenSolMass_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qAitkenSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qAitkenSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqAitkenSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqAitkenSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qAccumSolMass), "f_init_pl_qAccumSolMass")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qAccumSolMass(1:size(f_init_pl_qAccumSolMass)), &
          zgrid, current_state%global_grid%configuration%vertical%qAccumSolMass_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qAccumSolMass%data(:,j,i) = current_state%global_grid%configuration%vertical%qAccumSolMass_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qAccumSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qAccumSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqAccumSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqAccumSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qAccumInsolMass), "f_init_pl_qAccumInsolMass")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qAccumInsolMass(1:size(f_init_pl_qAccumInsolMass)), &
          zgrid, current_state%global_grid%configuration%vertical%qAccumInsolMass_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qAccumInsolMass%data(:,j,i) = current_state%global_grid%configuration%vertical%qAccumInsolMass_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qAccumInsolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qAccumInsolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqAccumInsolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqAccumInsolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qCoarseSolMass), "f_init_pl_qCoarseSolMass")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qCoarseSolMass(1:size(f_init_pl_qCoarseSolMass)), &
          zgrid, current_state%global_grid%configuration%vertical%qCoarseSolMass_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qCoarseSolMass%data(:,j,i) = current_state%global_grid%configuration%vertical%qCoarseSolMass_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qCoarseSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qCoarseSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqCoarseSolMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqCoarseSolMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
       ...
       call check_top(zztop, z_init_pl_q(size(z_init_pl_q)), 'z_init_pl_q')
       call check_input_levels(size(z_init_pl_q), size(f_init_pl_qCoarseDustMass), "f_init_pl_qCoarseDustMass")
       zgrid=current_state%global_grid%configuration%vertical%zn(:)
       call piecewise_linear_1d(z_init_pl_q(1:size(z_init_pl_q)), f_init_pl_qCoarseDustMass(1:size(f_init_pl_qCoarseDustMass)), &
          zgrid, current_state%global_grid%configuration%vertical%qCoarseDustMass_init)
       if (.not. current_state%continuation_run) then
         do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
           do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
             current_state%qCoarseDustMass%data(:,j,i) = current_state%global_grid%configuration%vertical%qCoarseDustMass_init(:)
           end do
         end do
       else
        call check(nf90_inq_varid(ncdf_id, "qCoarseDustMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%qCoarseDustMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zqCoarseDustMass", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zqCoarseDustMass%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    if (l_init_pl_n) then
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nl), "f_init_pl_nl")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nl(1:size(f_init_pl_nl)), &
         zgrid, current_state%global_grid%configuration%vertical%nl_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nl%data(:,j,i) = current_state%global_grid%configuration%vertical%nl_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nl", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nl%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znl", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znl%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nr), "f_init_pl_nr")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nr(1:size(f_init_pl_nr)), &
         zgrid, current_state%global_grid%configuration%vertical%nr_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nr%data(:,j,i) = current_state%global_grid%configuration%vertical%nr_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nr", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nr%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znr", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znr%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_ni), "f_init_pl_ni")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_ni(1:size(f_init_pl_ni)), &
         zgrid, current_state%global_grid%configuration%vertical%ni_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%ni%data(:,j,i) = current_state%global_grid%configuration%vertical%ni_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "ni", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%ni%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zni", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zni%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_ns), "f_init_pl_ns")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_ns(1:size(f_init_pl_ns)), &
         zgrid, current_state%global_grid%configuration%vertical%ns_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%ns%data(:,j,i) = current_state%global_grid%configuration%vertical%ns_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "ns", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%ns%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zns", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zns%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_ng), "f_init_pl_ng")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_ng(1:size(f_init_pl_ng)), &
         zgrid, current_state%global_grid%configuration%vertical%ng_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%ns%data(:,j,i) = current_state%global_grid%configuration%vertical%ng_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "ng", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%ng%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "zng", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%zng%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nAitkenSolNumber), "f_init_pl_nAitkenSolNumber")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nAitkenSolNumber(1:size(f_init_pl_nAitkenSolNumber)), &
         zgrid, current_state%global_grid%configuration%vertical%nAitkenSolNumber_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nAitkenSolNumber%data(:,j,i) = current_state%global_grid%configuration%vertical%nAitkenSolNumber_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nAitkenSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nAitkenSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znAitkenSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znAitkenSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nAccumSolNumber), "f_init_pl_nAccumSolNumber")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nAccumSolNumber(1:size(f_init_pl_nAccumSolNumber)), &
         zgrid, current_state%global_grid%configuration%vertical%nAccumSolNumber_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nAccumSolNumber%data(:,j,i) = current_state%global_grid%configuration%vertical%nAccumSolNumber_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nAccumSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nAccumSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znAccumSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znAccumSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nAccumInsolNumber), "f_init_pl_nAccumInsolNumber")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nAccumInsolNumber(1:size(f_init_pl_nAccumInsolNumber)), &
         zgrid, current_state%global_grid%configuration%vertical%nAccumInsolNumber_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nAccumInsolNumber%data(:,j,i) = current_state%global_grid%configuration%vertical%nAccumInsolNumber_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nAccumInsolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nAccumInsolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znAccumInsolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znAccumInsolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nCoarseSolNumber), "f_init_pl_nCoarseSolNumber")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nCoarseSolNumber(1:size(f_init_pl_nCoarseSolNumber)), &
         zgrid, current_state%global_grid%configuration%vertical%nCoarseSolNumber_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nCoarseSolNumber%data(:,j,i) = current_state%global_grid%configuration%vertical%nCoarseSolNumber_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nCoarseSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nCoarseSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znCoarseSolNumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znCoarseSolNumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
      ...
      call check_top(zztop, z_init_pl_n(size(z_init_pl_n)), 'z_init_pl_n')
      call check_input_levels(size(z_init_pl_n), size(f_init_pl_nCoarseDustnumber), "f_init_pl_nCoarseDustnumber")
      zgrid=current_state%global_grid%configuration%vertical%zn(:)
      call piecewise_linear_1d(z_init_pl_n(1:size(z_init_pl_n)), f_init_pl_nCoarseDustnumber(1:size(f_init_pl_nCoarseDustnumber)), &
         zgrid, current_state%global_grid%configuration%vertical%nCoarseDustnumber_init)
      if (.not. current_state%continuation_run) then
        do i=current_state%local_grid%local_domain_start_index(X_INDEX), current_state%local_grid%local_domain_end_index(X_INDEX)
          do j=current_state%local_grid%local_domain_start_index(Y_INDEX), current_state%local_grid%local_domain_end_index(Y_INDEX)
            current_state%nCoarseDustnumber%data(:,j,i) = current_state%global_grid%configuration%vertical%nCoarseDustnumber_init(:)
          end do
        end do
      else
        call check(nf90_inq_varid(ncdf_id, "nCoarseDustnumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%nCoarseDustnumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
            ...
            count=[1, current_state%local_grid%size(Z_INDEX), current_state%local_grid%size(Y_INDEX), &
            current_state%local_grid%size(X_INDEX)]))
        call check(nf90_inq_varid(ncdf_id, "znCoarseDustnumber", variable_id))
        call check(nf90_get_var(ncdf_id, variable_id, current_state%znCoarseDustnumber%data( &
            current_state%local_grid%local_domain_start_index(Z_INDEX):current_state%local_grid%local_domain_end_index(Z_INDEX), &
            current_state%local_grid%local_domain_start_index(Y_INDEX):current_state%local_grid%local_domain_end_index(Y_INDEX), &
            current_state%local_grid%local_domain_start_index(X_INDEX):current_state%local_grid%local_domain_end_index(X_INDEX)),&
            start=[1, current_state%local_grid%start(Z_INDEX), current_state%local_grid%start(Y_INDEX), &
    ...
    !else !(casim)
    !if (l_init_pl_q) then
    !  call log_master_log(LOG_ERROR, "l_init_pl_q activated but not Casim ! Casim required!")
    !end if
    !if (l_init_pl_n) then
    !  call log_master_log(LOG_ERROR, "l_init_pl_n activated but not Casim ! Casim required!")
    !end if
    !end if !(casim)
    ...
    !print*,"n_tracers = ",current_state%n_tracers
  ...
  subroutine set_buoyancy_coefficient(current_state, vertical_grid, kkp)
    ...
    if (.not. current_state%passive_th) then
      if(current_state%use_anelastic_equations)then ! aneslatic
        do k=1, kkp-1
  ...
  subroutine setup_reference_state_liquid_water_temperature_and_saturation(current_state, vertical_grid, kkp)
   ...
   if (current_state%th%active) then
      ...
      do k = 1,kkp
         vertical_grid%prefrcp(k)=(current_state%surface_reference_pressure/vertical_grid%prefn(k))**r_over_cp ! (Psuf/Pz)^rcp
         vertical_grid%rprefrcp(k)=1.0_DEFAULT_PRECISION/vertical_grid%prefrcp(k) ! values to return from potential temperature (K) to temperature (K)
      ...
      !if (current_state%calculate_th_and_q_init) then ! à revoir
...
!       vertical_grid%theta_init(k)=vertical_grid%thref(k) ! In LEM this can also be set from configuration (TODO)       
  ...
  subroutine set_up_and_smooth_grid(current_state, vertical_grid, z_size, zztop, nsmth, continuation_run)!Lambert
    type(model_state_type), intent(inout) :: current_state
    type(vertical_grid_configuration_type), intent(inout) :: vertical_grid
    integer, intent(in) :: nsmth, z_size
    real(kind=DEFAULT_PRECISION),intent(in) :: zztop
    logical, intent(in) :: continuation_run

    real(kind=DEFAULT_PRECISION), dimension(:), allocatable :: spe_vertical_grid_reso ! special vertical grid resolution given by user
    integer :: k, iter, iter2, size_array

    !if (.not. continuation_run) then
    if (current_state%origional_vertical_grid_setup) then
      call original_vertical_grid_setup(vertical_grid, z_size, zztop, nsmth)!Lambert
    else if (current_state%new_vertical_grid_setup) then
      call new_vertical_grid_setup(vertical_grid, z_size, zztop)!Lambert
    else if (current_state%special_vertical_grid_setup) then
      do iter = 1,current_state%config_args
        size_array = 0
        if (current_state%options_database_string(iter,1) .eq. "special_vertical_grid_resolution") then
          do iter2 = 1,size(current_state%options_database_real(iter,:))
            if (isnan(current_state%options_database_real(iter,iter2))) then
              exit
            else
          ...
          allocate(spe_vertical_grid_reso(size_array))
          spe_vertical_grid_reso = current_state%options_database_real(iter,1:size_array)
          vertical_grid%z(:) = spe_vertical_grid_reso(:)
        end if
      end do
      do k=2,z_size
      ...
      vertical_grid%zn(z_size)=0.5_DEFAULT_PRECISION*(vertical_grid%z(z_size-1)+vertical_grid%z(z_size))
    end if
    !end if
  ...
  !! @param zztop The real world (m) height of the top of the column
  !! @param nsmth Number of smoothing iterations to run on the grid
  subroutine original_vertical_grid_setup(vertical_grid, z_size, zztop, nsmth)
    type(vertical_grid_configuration_type), intent(inout) :: vertical_grid
    integer, intent(in) :: nsmth, z_size
    real(kind=DEFAULT_PRECISION), intent(in) :: zztop
    ...
    !if (allocated(vertical_grid%origional_vertical_grid_interp_levels)) then
    call create_linear_grid(vertical_grid, z_size, zztop)
    ! Smooth grid
    vertical_grid%z(1)=0.0_DEFAULT_PRECISION
    vertical_grid%z(z_size)=zztop
    do n=1,nsmth
      do k=2,z_size
        vertical_grid%zn(k)=0.5_DEFAULT_PRECISION*(vertical_grid%z(k)+vertical_grid%z(k-1))
      end do
  ...
  !! @param kkp Number of points in the vertical domain
  !! @param zztop The real world (m) height of the top of the column
  subroutine new_vertical_grid_setup(vertical_grid, z_size, zztop)
    type(vertical_grid_configuration_type), intent(inout) :: vertical_grid
    integer, intent(in) :: z_size
    real(kind=DEFAULT_PRECISION), intent(in) :: zztop

    real(kind=DEFAULT_PRECISION) :: a(2*z_size), r1, d1, dd, d0, a0
    ...
    do k=3, z_size*2
      if (d0 .gt. dd .or. k==3) then
        ...
        d0=(zztop-a(k))/real(z_size*2-k, kind=DEFAULT_PRECISION)
      else
        if (first_gt) then
          k0=k
          a0=a(k-1)+d0
  ...
  !! @param zztop The real world (m) height of the top of the column
  !! @param nsmth Number of smoothing iterations to run on the grid
  subroutine create_linear_grid(vertical_grid, z_size, zztop)
    type(vertical_grid_configuration_type), intent(inout) :: vertical_grid
    real(kind=DEFAULT_PRECISION), intent(in) :: zztop
    integer, intent(in) :: z_size
    ...
    allocate(kgd(size(vertical_grid%origional_vertical_grid_interp_levels)))
    allocate(hgd(size(vertical_grid%origional_vertical_grid_interp_altitudes)))

    kgd = vertical_grid%origional_vertical_grid_interp_levels
    hgd = vertical_grid%origional_vertical_grid_interp_altitudes

    !if ((size(kgd)) .eq. 1) then
    !  do i=2,kgd(1)
    !    vertical_grid%z(i) = vertical_grid%z(i-1) + (hgd(1)/(kgd(1)-1))
    !  end do
    !  do i=kgd(1)+1,z_size
    !    vertical_grid%z(i) = vertical_grid%z(i-1) + (zztop/((z_size-kgd(1))-1))
    !  end do
    !else if ((size(kgd)) .gt. 1) then
    !  k=1
    !  do i=2,kgd(k)
    !    vertical_grid%z(i) = vertical_grid%z(i-1) + (hgd(k)/(kgd(k)-1))
    !  end do

    !  do while (k .lt. size(kgd))
    !    do i=kgd(k)+1,kgd(k+1)
    !      vertical_grid%z(i) = vertical_grid%z(i-1) + ((hgd(k+1)-hgd(k))/((kgd(k+1)-kgd(k))))
    !    end do
    !    k = k+1
    !  end do
    !  do i=kgd(k)+1,z_size
    !    vertical_grid%z(i) = vertical_grid%z(i-1) + ((zztop -hgd(k)) /((z_size-kgd(k))))
    !  end do
    !end if
    !do k=1,size(kgd)
    !  if ((kgd(k) .gt. 1) .and. (k .lt. size))
    !  i=2,kgd(k)
    !  vertical_grid%z(i) = vertical_grid%z(i-1) + (hgd(k)/(kgd(k)-1))
    !end do
    ...
    if(kmax .lt. z_size)then
      do k=kmax,z_size
        ! Handle any points above the kth max division
        vertical_grid%z(k)=zmax+(zztop-zmax)*real(k-kmax, kind=DEFAULT_PRECISION)/real(z_size-kmax, kind=DEFAULT_PRECISION)
      end do
    end if
  ...
  subroutine allocate_vertical_grid_data(vertical_grid, n, nq)
    ...
    allocate(vertical_grid%dz(n), vertical_grid%dzn(n),&
         ...
         vertical_grid%rh_init(n), vertical_grid%qv_init(n), vertical_grid%ql_init(n), vertical_grid%qr_init(n), &
         vertical_grid%qi_init(n), vertical_grid%qs_init(n),vertical_grid%qg_init(n),&
         vertical_grid%qAitkenSolMass_init(n), vertical_grid%qAccumSolMass_init(n), vertical_grid%qAccumInsolMass_init(n), &
         vertical_grid%qCoarseSolMass_init(n), vertical_grid%qCoarseDustMass_init(n), &
         vertical_grid%nl_init(n), vertical_grid%nr_init(n), &
         vertical_grid%ni_init(n), vertical_grid%ns_init(n),vertical_grid%ng_init(n),&
         vertical_grid%nAitkenSolNumber_init(n), vertical_grid%nAccumSolNumber_init(n), vertical_grid%nAccumInsolNumber_init(n), &
         vertical_grid%nCoarseSolNumber_init(n), vertical_grid%nCoarseDustnumber_init(n), &
         ...
         vertical_grid%qv_rand(n), vertical_grid%ql_rand(n), vertical_grid%qr_rand(n), vertical_grid%qi_rand(n), &
         vertical_grid%qs_rand(n), vertical_grid%qg_rand(n), vertical_grid%qAitkenSolMass_rand(n), &
         vertical_grid%qAccumSolMass_rand(n), vertical_grid%qAccumInsolMass_rand(n), &
         vertical_grid%qCoarseSolMass_rand(n), vertical_grid%qCoarseDustMass_rand(n), &
         vertical_grid%qv_force(n), vertical_grid%ql_force(n), vertical_grid%qr_force(n), vertical_grid%qi_force(n), &
         vertical_grid%qs_force(n), vertical_grid%qg_force(n), &
         vertical_grid%qAitkenSolMass_force(n), vertical_grid%qAccumSolMass_force(n), vertical_grid%qAccumInsolMass_force(n), &
         vertical_grid%qCoarseSolMass_force(n), vertical_grid%qCoarseDustMass_force(n), vertical_grid%w_init(n), &
