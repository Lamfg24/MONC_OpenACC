...
module optionsdatabase_mod
  ...
  subroutine load_command_line_into_options_database(state, &
                                                     report_option)
    !type(hashmap_type), intent(inout) :: options_database
    ...
    do i=1,arguments
      ...
      if (index(specific_arg, "--") .eq. 1) then
        if (equals_posn .gt. 0) then
          ...
          state%options_database_string(i,1) = specific_arg(3:equals_posn-1)
          state%options_database_string(i,2) = specific_arg(equals_posn+1 : len(specific_arg))
        else
        ...
        state%options_database_real(i,1) = type_of_config
