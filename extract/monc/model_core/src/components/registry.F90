...
module registry_mod
  ...
  !procedure (component_init), pointer :: init_callback_petsc_solver_ptr => no_component
  ...
  procedure (component_init), pointer :: initialisation_callback_flux_budget_ptr => no_component
  procedure (component_init), pointer :: initialisation_callback_diagnostics_averaged_ptr => no_component
  ...
  !procedure (component_final), pointer :: finalisation_callback_petsc_solver_ptr => no_component
  ...
  procedure (component_final), pointer :: finalisation_callback_flux_budget_ptr => no_component
...
contains
  ...
  subroutine register_component(current_state)
    ...
    do iter = 1,current_state%config_args
      ...
      !  read(current_state%options_database_string(iter,2),*) logicnum
      !  if (logicnum .eqv. .true.) then
      !    init_callback_petsc_solver_ptr => init_callback_petsc_solver
      !    finalisation_callback_petsc_solver_ptr => finalisation_callback_petsc_solver
      !  end if
        ...
        read(current_state%options_database_string(iter,2),*) logicnum
        if (logicnum .eqv. .true.) then
          initialisation_callback_flux_budget_ptr => initialisation_callback_flux_budget
          finalisation_callback_flux_budget_ptr => finalisation_callback_flux_budget
        end if
        ...
        read(current_state%options_database_string(iter,2),*) logicnum
        if (logicnum .eqv. .true.) initialisation_callback_diagnostics_averaged_ptr => initialisation_callback_diagnostics_averaged
      end if
    end do
...
!     call c_put_generic(component_descriptions, descriptor%name, description_data, .false.)
  ...
  subroutine execute_initialisation_callbacks(current_state)
    ...
    !call init_callback_petsc_solver_ptr(current_state)
    ...
    if (current_state%init_to_apply_random_noise .eqv. .true.) then
    ...
    call initialisation_callback_flux_budget_ptr(current_state)
    call initialisation_callback_diagnostics_averaged_ptr(current_state)
  ...
  subroutine execute_finalisation_callbacks(current_state)
    ...
    !call finalisation_callback_petsc_solver_ptr(current_state)
    ...
    call finalisation_callback_flux_budget_ptr(current_state)
