...
module reduction_inter_io_mod
  ...
  use mpi, only : MPI_DOUBLE_PRECISION, MPI_INT, MPI_ANY_SOURCE, MPI_REQUEST_NULL, MPI_STATUS_IGNORE, MPI_CHARACTER, &
...
contains
  ...
  subroutine perform_inter_io_reduction(io_configuration, field_values, field_size, reduction_field_name, reduction_op, &
    ...
    !print*,"perform_inter_io_reduction0 and timestep =", timestep
    ...
    !call check_thread_status(forthread_mutex_lock(reduction_progress%mutex)) ! LAMBERT
    reduction_progress%contributed_moncs=reduction_progress%contributed_moncs+1
    
    !print*,"perform_inter_io_reduction_marker1"
    ...
    if (reduction_progress%contributed_moncs == io_configuration%number_of_moncs) then
      ...
      !print*,"perform_inter_io_reduction_marker2"
    ...
    !print*,"perform_inter_io_reduction1 and timestep =", timestep
  ...
  subroutine handle_local_moncs_completed_collective(io_configuration, reduction_progress)
    ...
    !print*,"handle_local_moncs_completed_collective000000000000000000000000000000"
    ...
    !print*,"handle_local_moncs_completed_collective11111111111111111111111111111111111111"
  ...
  subroutine handle_collective_completed(io_configuration, reduction_progress)
    type(io_configuration_type), intent(inout) :: io_configuration
    type(reduction_progress_type), intent(inout) :: reduction_progress
    integer :: request, ierr, tag_close_diag=0
    
    !print*,"handle_collective_completed000000000000000000000"
    ...
    !print*,"handle_collective_completed_aaaaa"
    ...
    !print*,"handle_collective_completed_bbbbb"
    call check_thread_status(forthread_mutex_unlock(reduction_progress%mutex))
    !print*,"handle_collective_completed_ccccc"
    !call check_thread_status(forthread_mutex_destroy(reduction_progress%mutex))
    !print*,"handle_collective_completed_ddddd"
    if (allocated(reduction_progress%values)) deallocate(reduction_progress%values)
    call remove_reduction_progress(reduction_progress)
    !print*,"handle_collective_completed111111111111111111111"
  ...
  function find_or_add_reduction_progress(timestep, reduction_operator, root, field_name, completion_procedure)
    ...
    !print*,"find_or_add_reduction_progress0 and timestep = ",timestep
    ...
    if (.not. associated(find_or_add_reduction_progress)) then
      ...
      if (.not. associated(find_or_add_reduction_progress)) then
        ...
        !print*,"find_or_add_reduction_progress0 and new_progress%mutex = ", new_progress%mutex
    ...
    !print*,"find_or_add_reduction_progress1 and timestep = ",timestep
  ...
  subroutine remove_reduction_progress(reduction_progress)
    ...
    !print*,"remove_reduction_progress00000"
    ...
    !print*,"remove_reduction_progress11111"
    call check_thread_status(forthread_rwlock_wrlock(reduction_progress_rwlock))
    !print*,"remove_reduction_progress222222"
    generic=>c_get_generic(reduction_progresses, specific_key)
    !print*,"remove_reduction_progress333333"
    call c_remove(reduction_progresses, specific_key)
    !print*,"remove_reduction_progress444444"
    call check_thread_status(forthread_rwlock_unlock(reduction_progress_rwlock))
    !print*,"remove_reduction_progress555555"
