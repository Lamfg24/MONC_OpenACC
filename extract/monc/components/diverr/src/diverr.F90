...
module diverr_mod
  ...
  public init_callback_diverr, timestep_callback_diverr, &
         finalisation_callback_diverr
contains

  !> The initialisation callback will allocate memory for the P field and initialise it
  !! @param current_state The current model state
  subroutine init_callback_diverr(current_state)
  ...
  end subroutine init_callback_diverr

  !> Called each timestep this will initialise P and update the local divergence error for the current column
  !! @param current_state The current model state
  subroutine timestep_callback_diverr(current_state)
    type(model_state_type), target, intent(inout) :: current_state

    current_state%p%data(:, current_state%column_local_y, current_state%column_local_x) = 0.0_DEFAULT_PRECISION ! need to reset p variable to zero for the pressure solver
  ...
  subroutine calculate_p(current_state, p, u, v, w, timec, y_local, x_local)
    ...
    if(current_state%immersed%ib_enabled .eqv. .true.) then
      if (current_state%immersed%ib_col(y_local, x_local) .eqv. .true.) then
      ! IB enabled
      do k=2,current_state%local_grid%size(Z_INDEX)
#ifdef U_ACTIVE
