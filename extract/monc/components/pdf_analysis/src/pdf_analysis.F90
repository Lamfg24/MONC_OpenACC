...
contains
  ...
  subroutine init_callback_pdf_analysis(current_state)
    ...
    allocate( current_state%w_histogram_profile_local( current_state%local_grid%size(Z_INDEX), current_state%n_w_bins) )
    allocate( w_histogram_bins(current_state%n_w_bins) )
    w_histogram_bins(1) = w_bin_min
    do inc = 2, current_state%n_w_bins
  ...
  subroutine finalisation_callback_pdf_analysis(current_state)
    ...
    if (allocated(current_state%w_histogram_profile_local)) deallocate(current_state%w_histogram_profile_local)
  ...
  subroutine calculate_w_percentiles(current_state)
    ...
    do k = 2, current_state%local_grid%size(Z_INDEX)
       ...
       do bnc = bpn, bpx
         current_state%w_histogram_profile_local(k, bnc) =                             &
