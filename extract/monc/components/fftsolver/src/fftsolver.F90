...
module fftsolver_mod
  ...
  public initialisation_callback_fftsolver, timestep_callback_fftsolver, finalisation_callback_fftsolver
contains
  ...
  subroutine initialisation_callback_fftsolver(current_state)
    type(model_state_type), target, intent(inout) :: current_state

    integer :: i, my_y_start, my_x_start
    
    ! filter for immersed boundary
    integer :: j, x_size, y_size, iter, intnum, f_kmax
    real(kind=DEFAULT_PRECISION) :: tmp1, tmp2, tmp3, fn, ff, cutoff, realnum, f_cutoff, f_order
    logical :: l_tmp, logicnum, diverr_is_enabled, filter_is_enabled
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "diverr_enabled") then
        ...
        diverr_is_enabled = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "diverr_enabled") then
        read(current_state%options_database_string(iter,2),*) logicnum
        filter_is_enabled = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "f_cutoff") then
        read(current_state%options_database_string(iter,2),*) realnum
        f_cutoff = realnum
      else if (current_state%options_database_string(iter,1) .eq. "f_order") then
        read(current_state%options_database_string(iter,2),*) realnum
        f_order = realnum
      else if (current_state%options_database_string(iter,1) .eq. "f_kmax") then
        read(current_state%options_database_string(iter,2),*) intnum
        f_kmax = intnum
      end if
    end do

    if (.not. diverr_is_enabled) then
    ...
    if (current_state%immersed%ib_enabled) then
      l_tmp=filter_is_enabled
      if(l_tmp)filter_enabled=.true.
  ...
  end subroutine finalisation_callback_fftsolver
