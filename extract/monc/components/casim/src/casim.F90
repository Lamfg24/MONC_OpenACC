...
module casim_mod
  ...
  use mphys_switches, only: set_mphys_switches,  &
     ...
     , l_phomc_wisener & ! homogeneous freezing of cloud droplets Wisener scheme 1972
     , l_phomc_seifert & ! homogeneous freezing of cloud droplets Seifert scheme 2006
...
contains
  ...
  subroutine initialisation_callback_casim(current_state)
   ...
   !      pcond_tot(current_state%local_grid%size(Z_INDEX)))
    ...
    if (.not. l_warm) then
       ...
       if ( l_phomc_wisener .or.  l_phomc_seifert) casdiags % l_phomc = .TRUE.
  ...
  subroutine timestep_callback_casim(current_state)
    ...
    pressure(:,1,1) = current_state%global_grid%configuration%vertical%prefn(:) !  ATTENTION PREFN NOT GOOD in vn1.0.0
  ...
  subroutine read_configuration(current_state)
    ...
    do iter = 1,current_state%config_args
      if (current_state%options_database_string(iter,1) .eq. "option") then
        ...
        read(current_state%options_database_string(iter,2),*) logicnum
        l_phomr = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "l_phomc_wisener") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_phomc_wisener = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "l_phomc_seifert") then
        read(current_state%options_database_string(iter,2),*) logicnum
        l_phomc_seifert = logicnum
