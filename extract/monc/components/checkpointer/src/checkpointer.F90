...
module checkpointer_mod
  ...
  public checkpointer_get_descriptor, initialisation_callback_checkpointer, timestep_callback_checkpointer, &
         finalisation_callback_checkpointer

contains

  !> Provides registry information for the component
  !! @returns The component descriptor that describes this component
  type(component_descriptor_type_v1) function checkpointer_get_descriptor()
    checkpointer_get_descriptor%name="checkpointer"
    checkpointer_get_descriptor%version=0.1
    checkpointer_get_descriptor%published_fields_on_off = .false.
  ...
  subroutine initialisation_callback_checkpointer(current_state)
    ...
    do iter = 51,600
      if (current_state%options_database_string(iter,1) .eq. "checkpoint_frequency") then
        read(current_state%options_database_string(iter,2),*) intnum
        checkpoint_frequency = intnum
      else if (current_state%options_database_string(iter,1) .eq. "checkpoint_file") then
        checkpoint_file = current_state%options_database_string(iter,2)
      else if (current_state%options_database_string(iter,1) .eq. "checkpoint_unique_per_dump") then
        read(current_state%options_database_string(iter,2),*) logicnum
        unique_per_dump = logicnum
      else if (current_state%options_database_string(iter,1) .eq. "checkpoint_internal_write") then
        internal_write_mode = current_state%options_database_string(iter,2)
      end if
    end do
    ...
    !end if
  end subroutine initialisation_callback_checkpointer

  !> The timestep hook will dump out model state_mod to a checkpoint file
  !! @param current_state The current model state
  subroutine timestep_callback_checkpointer(current_state)
    type(model_state_type), target, intent(inout) :: current_state
  ...
  end subroutine timestep_callback_checkpointer

  !> Called on termination to write out the status of the model run to checkpoint
  !! @param current_state The current model state
  subroutine finalisation_callback_checkpointer(current_state)
    type(model_state_type), target, intent(inout) :: current_state

    if (enable_write) call perform_checkpoint_dump(current_state)
  end subroutine finalisation_callback_checkpointer
  ...
  subroutine perform_checkpoint_dump(current_state)
    ...
    if (unique_per_dump) then !à revoir
