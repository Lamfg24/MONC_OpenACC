...
contains
  ...
  !
  subroutine examine_aerosol(ntimestep, aerofields, qfields, aerophys, aerochem, aeroact, &
...
!   real(wp) :: mratio_i, mratio_s, mratio_g
  ...
  subroutine AbdulRazzakGhan2000(w, p, T, phys, chem, nccn, Smax, active_phys, nccn_active, l_useactive )
    ...
    es=(100.0*6.1121)*exp((18.678-Tc/(234.5))*Tc/(257.14+Tc)) ! saturation vapor pressure Buck_1981
  ...
  subroutine AbdulRazzakGhan2000_dust(ntimestep, w, p, T, phys, chem, nccn, Smax, active_phys, nccn_active, &
    ...
    if (phys%N(aero_index%i_accum) .eq. 0.0 .and. phys%N(aero_index%i_coarse) .eq. 0.0) return
    IF (lhook) CALL dr_hook(ModuleName//':'//RoutineName,zhook_in,zhook_handle)
    ...
    do i=1, phys%nmodes
      if (phys%N(i) > ccn_tidy .and. phys%M(i) > aeromass_small) then ! LAMBERT better constrai
        if(l_ukca_casim) then
          Bk =chem%bk(i)
        else
          Bk=chem%vantHoff(i)*Mw*chem%density(i)/(chem%massMole(i)*rhow)
    ...
    do i=1,dphys%nmodes
      if (opt_karcher .eqv. .true.) then
        if (i.eq. aero_index%i_coarse_dust) then ! vincent
          continue
        else
          if (dphys%N(i) > ni_tidy  .and. dphys%M(i) > aeromass_small) then ! LAMBERT better constrain
    ...
    do i=1,dphys%nmodes
      if (opt_karcher .eqv. .true.) then
        if (i.eq. aero_index%i_coarse_dust) then ! vincent
          ...
          if (dphys%N(i) > ni_tidy .and. dphys%M(i) > aeromass_small) then ! LAMBERT better constrai
