...
!!!NB if we keep l_use_critical_w approach need to deal with dnumber=/=dnumberi for processing!!
  ...
  ! homogeneous freezing scheme Seifert and Beheng (2006) from  Lüttmer et al (2025)
  subroutine ihom_droplets_SB(dt, nz, l_Tcold, qfields, aeroact, dustliq, procs, aerosol_procs)
...
!    real(wp) :: m1, m2, m3
    ...
    real(wp) :: th
    real(wp) :: ql, nl
    real(wp) :: J_hom ! homogeneous freezing coefficient
    real(wp) :: xl ! mean mass of liquid water
    real(wp) :: nu_factor ! gamma law shape parameter nu factor

    real(wp) :: Tc, Tk, common_T_calc, dependant_T_calc, T_calc

    logical :: l_condition

    integer :: k

    character(len=*), parameter :: RoutineName='IHOM_DROPLETS_SB'
    ...
    do k = 1, nz
      ql = qfields(k, i_ql)
      nl = qfields(k, i_nl)
      if (l_Tcold(k) .and. ql .gt. 0.0_wp .and. nl .gt. 0.0_wp) then
        th = qfields(k, i_th)
        Tc = th*exner(k) - 273.15
        Tk = th*exner(k)

        if (Tc .le. 30.0) then
          T_calc = -243.4 - 14.75*Tc - 0.307*(Tc**2) - 0.00287*(Tc**3) - 0.0000102*(Tc**4)
        else if (Tc .gt. 30.0) then
          T_calc = -7.63 - 2.996*(Tc + 30)
        end if

        !T_calc = common_T_calc + dependant_T_calc
        !print*,"Tc = ",Tc," T_calc = ",T_calc," 10**T_calc = ",10**T_calc
        J_hom = exp(T_calc)


        xl = ql/nl
        nu_factor = (cloud_params%gam_1_mu + 2)/(cloud_params%gam_1_mu + 1)

        !print*,"J_hom = ",J_hom," nu_factor = ",nu_factor
        dmass = J_hom*ql*xl*nu_factor*dt
        dnumber = J_hom*ql*dt

        !print*,"Tc = ",Tc," J_hom = ",J_hom," dmass = ",dmass," nl = ",nl," dnumber = ",dnumber
        !print*,"dmass = ",dmass," ql = ",ql, "dnumber = ",dnumber," nl = ",nl
        if (dmass .gt. ql) dmass = ql
        if (dnumber .gt. nl) dnumber = nl
  ...
  end subroutine ihom_droplets_SB
