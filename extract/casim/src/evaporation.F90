...
! use mphys_switches, only: i_m3r, l_3mr
  use mphys_switches, only: i_qr, i_nr, i_qv, i_ql, i_nl, l_2mr, i_am2, &
    ...
    do k = 1, nz
       ...
       if (qv/qws(k) < 1.0-ss_small .and. qfields(k, i_ql) < ql_tidy .and. rain_mass > qr_tidy) then
          ...
          if (l_2mr) then
             ...
             !print*,-dnumber
             !open(unit=stdout, file="/dev/stdout", status="old") ! PROVISOIRE : Heisenbug à debogger
          end if
          ! if (l_3mr) then
