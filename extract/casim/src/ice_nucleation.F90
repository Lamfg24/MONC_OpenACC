...
contains
  ...
  subroutine inuc(dt, nz, l_Tcold, qfields, cffields, procs, dustphys, aeroact, dustliq, &
    ...
    ! parameters for Vincent soot
    real(wp) :: a_soot, b_soot
    real(wp) :: soot_actrate
             ...
             case (10)
              ...
              case (11)  ! Vincent Karcher case
                ...
                if (dustphys(k)%N(1) > ni_tidy) then
                  ...
                  soot_actrate = 10**(1 / (a_soot + b_soot*(Si+1))) !Karcher et al. 2021
                  dN_contact = soot_actrate * dustphys(k)%N(1)
                endif
             
             case (12) ! Vincent Righi case
                ...
                if (dustphys(k)%N(1) > ni_tidy) then
                  if(Si<0.4) then
                    ...
                    a_soot=1.0d-4
                    b_soot=46.05d0
                    soot_actrate=a_soot*exp(b_soot*(Si-1.40d0))
                  endif
